"""Prepare the frozen 64-receptor PPARA cognate-redocking input pool."""
...
def validate_inputs(
    ...
    if file_sha256(Path(__file__)) != str(config["implementation"]["sha256"]).upper():
        raise ValueError("Stage50 implementation SHA-256 differs")
    ...
    selection = read_json(inputs["stage49b_result"])
    audit = read_json(inputs["stage49b_audit"])
    if selection.get("status") != "stage49b_ppara_structural_pool_ok":
        raise ValueError("Stage49b PPARA structural pool did not pass")
    if audit.get("status") != "stage49b_ppara_structural_pool_independent_audit_ok":
        raise ValueError("Stage49b independent audit did not pass")
    rows = read_csv(inputs["selected_receptor_manifest"])
    expected = int(config["expected"]["receptor_count"])
    if len(rows) != expected or len({row["conformer_id"] for row in rows}) != expected:
        raise ValueError("Stage50 frozen receptor identities differ")
    if [row["conformer_id"] for row in rows] != selection["selected_receptor_ids"]:
        raise ValueError("Stage50 receptor order differs from Stage49b")
    if rows[0]["conformer_id"] != "PPARA_2P54_reference":
        raise ValueError("Stage50 reference receptor is not first")
    if len(
        {
        ...
        raise ValueError("Stage50 cognate cases are not unique")
    for row in rows:
        ...
        if row["explicit_covalent_connections"].strip():
            raise ValueError("Stage50 selected pool contains an explicit covalent ligand")
    boundary = {
...
def checkpoint(
    ...
    try:
        ...
        if record.get("status") != "stage50_case_preparation_ok":
            return None
...
def prepare_case(
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"PPARA receptor preparation failed with return code {completed.returncode}")
    ...
    if receptor_evidence.get("status") != "ok" or receptor_evidence.get("allow_bad_res") is not False:
        raise ValueError(f"PPARA receptor preparation evidence failed: {conformer_id}")
    if float(receptor_evidence["maximum_existing_atom_displacement_angstrom"]) > float(
        config["preparation_protocol"]["maximum_existing_atom_displacement_angstrom"]
    ):
        raise ValueError(f"PPARA completion moved existing atoms: {conformer_id}")
    residue_change = dict(receptor_evidence["residue_count_change"])
    if len(set(residue_change.values())) != 1:
        raise ValueError(f"PPARA receptor residue count changed: {conformer_id}")
    ...
    if ligand_key not in raw_ligands:
        raise ValueError(f"selected PPARA mmCIF ligand is missing: {case_id}")
    ...
    if raw_molecule.GetNumHeavyAtoms() != int(selected["selected_ligand_heavy_atom_count"]):
        raise ValueError(f"PPARA ligand heavy-atom count differs: {case_id}")
    raw_rmsd, raw_maximum = point_set_rmsd_by_element(
        raw_coordinates, raw_elements, mmcif_coordinates, mmcif_elements
    )
    limit = float(config["preparation_protocol"]["maximum_element_matched_coordinate_error_angstrom"])
    if raw_rmsd > limit or raw_maximum > limit:
        raise ValueError(f"PPARA ModelServer ligand coordinates differ: {case_id}")
    ...
    if common_rmsd > limit or common_maximum > limit:
        raise ValueError(f"PPARA common-frame ligand coordinates differ: {case_id}")
    ...
    receptor_row = {
        ...
        "completed_receptor_pdb_sha256": file_sha256(completed_pdb),
    ...
    write_json(
        ...
        {
            "schema_version": "1.0",
            "status": "stage50_case_preparation_ok",
...
def run(
    ...
    load_dependencies()
    outputs = {key: rooted(root, str(value)) for key, value in config["outputs"].items()}
    ...
    for index, selected in enumerate(selected_rows, start=1):
        ...
        if cached is None:
            ...
            try:
                receptor, case, coordinates = prepare_case(
                ...
                print(f"preparation failed {index}/{len(selected_rows)}: {selected['conformer_id']}; continuing", flush=True)
                continue
        else:
    ...
    if not coordinate_sets:
        status = "stage50_ppara_large_pool_redocking_inputs_failed"
        result = {
            "schema_version": "1.0",
            "experiment_id": config["experiment_id"],
            "status": status,
    ...
    if max(box["size"].values()) > float(box_rule["maximum_axis_size_angstrom"]):
        raise ValueError("PPARA large-pool common box is too large")
    box_record = {
        "schema_version": "1.0",
        "status": "stage50_ppara_large_pool_common_box_ok",
    ...
    if len(receptor_rows) == len(selected_rows):
        status = "stage50_ppara_large_pool_redocking_inputs_ok"
    elif gate_ready:
        status = "stage50_ppara_large_pool_redocking_inputs_partial_gate_ready"
    else:
        status = "stage50_ppara_large_pool_redocking_inputs_failed"
    result = {
        "schema_version": "1.0",
        ...
        "next_gate": (
            "run frozen three-seed PPARA Uni-Dock cognate redocking for every successfully prepared receptor"
            if gate_ready
            else "stop PPARA because fewer than 24 frozen receptors were technically preparation-ready"
        ),
        "interpretation_boundary": config["interpretation_boundary"],
    }
    write_json(outputs["summary_json"], result)
