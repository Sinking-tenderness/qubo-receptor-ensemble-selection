"""Build the deterministic Stage 19i objective-screen core bundle."""
...
CONFIG = Path("configs/stage19i_objective_adequacy_noise_screen.json")
AUDIT = Path("data/stage19i_objective_adequacy_noise_screen_audit.json")
FIXED_PATHS = (
    "scripts/audit_stage19i_objective_adequacy_noise_screen.py",
    "scripts/build_stage05_mk14_remote_bundle.py",
    "scripts/build_stage19i_objective_adequacy_noise_screen_bundle.py",
    ...
    "tests/test_stage19i_objective_adequacy_noise_screen.py",
)
...
def bundle_paths(root: Path) -> list[str]:
    ...
    if result["status"] != (
        ...
        raise ValueError("Stage 19i result status differs")
    if audit["status"] != "stage19i_objective_adequacy_noise_screen_audit_ok":
        raise ValueError("Stage 19i audit did not pass")
    if audit["result"]["sha256"] != file_sha256(result_path):
        raise ValueError("Stage 19i audit identifies another result")
    if result["ready_candidates"]:
        raise ValueError("Stage 19i bundle cannot contain a ready hardware candidate")
    if result["data_boundary"]["quantum_hardware_jobs"] != 0:
        raise ValueError("Stage 19i bundle contains hardware jobs")
    ...
    if any(marker in path.lower() for path in normalized for marker in forbidden):
        raise ValueError("Stage 19i bundle contains a protected-panel path")
    return normalized
...
def main() -> int:
    ...
    result.update(
        {
            "operation": "Stage 19i objective adequacy and noise screen core results",
            "target_ids": ["MK14", "PPARG"],
            "experiment_class": "posthoc_cross_target_train_only_objective_design_review",
            "ready_candidates": [],
            "new_docking_jobs": 0,
