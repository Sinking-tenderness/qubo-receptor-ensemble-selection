"""Independently audit Stage82 aggregation and hardware authorization."""
...
def run(config_path: Path, root: Path) -> dict[str, Any]:
    ...
    aggregation_checks = {
        ...
        "target_fractions": all(
            ...
            <= 1e-12
            for target, fraction in target_fractions.items()
        ),
    }
    gate = config["decision_gate"]
    independent_gate_checks = {
    ...
    gate_passed = all(independent_gate_checks.values())
    checks = {
        "output_identity": all(identity_checks),
        "aggregation": all(aggregation_checks.values()),
        "gate_checks_match": independent_gate_checks == summary["gate_checks"],
        "gate_decision_consistent": gate_passed
    ...
    if not all(checks.values()):
        raise ValueError(f"Stage82 independent audit failed: {checks}")
    audit = {
        "schema_version": "1.0",
        "status": "stage82_lagrangian_fixed_k_qubo_independent_audit_ok",
        "trial_rows_audited": len(trials),
        "fixed_k_rows_audited": len(cells),
        "variable_k_rows_audited": len(variable),
        "checks": checks,
        "limited_qci_calibration_authorized": gate_passed,
