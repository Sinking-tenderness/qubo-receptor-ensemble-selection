"""Build the deterministic PPARG cognate-redocking GPU bundle."""
...
try:
    ...
    from scripts.experimental.unidock.run_stage18e_pparg_cognate_redocking import (
        common,
        validate_inputs,
    )
...
CONFIG = "configs/stage18e_pparg_cognate_redocking.json"
FIXED_PATHS = (
    CONFIG,
    ...
    "scripts/experimental/unidock/run_stage18e_pparg_cognate_redocking.py",
    "scripts/experimental/unidock/run_stage18e_pparg_cognate_redocking_remote.sh",
    "scripts/experimental/unidock/build_stage18e_pparg_cognate_redocking_bundle.py",
)
...
def bundle_paths(root: Path) -> list[str]:
    ...
    if audit["expected_redocking_pair_count"] != 72:
        raise ValueError("Stage 18e bundle pair count differs")
    if any(
        int(audit[key]) != 0
        for key in (
            "ligand_labels_read",
        ...
        raise ValueError("Stage 18e bundle crossed a data boundary")
    ...
    if any(marker in path.lower() for path in normalized for marker in forbidden):
        raise ValueError("Stage 18e bundle contains a forbidden result path")
    return normalized
...
def main() -> int:
    ...
    result.update({
        "operation": "Stage 18e PPARG 24-receptor three-seed cognate redocking bundle",
        "target_id": "PPARG",
        "receptor_count": 24,
        "cognate_ligand_count": 24,
        "seed_count": 3,
        "gpu_batch_count": 72,
        "gpu_pair_count": 72,
        "profile_id": "enhanced",
