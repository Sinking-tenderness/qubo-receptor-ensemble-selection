"""Run the Stage 07 train-only Uni-Dock search-profile sensitivity study."""
...
PROFILE_ORDER = ("fast", "balance", "detail")
...
def validate_config(config: dict[str, object]) -> None:
    ...
    if set(config) != required:
        raise ValueError("Stage 07 sensitivity config keys differ")
    profiles = dict(config["profiles"])
    if tuple(profiles) != PROFILE_ORDER:
        raise ValueError("profile order must be fast, balance, detail")
    expected_profiles = {
        "fast": (128, 20),
        "balance": (384, 40),
        "detail": (512, 40),
    ...
    for profile_id, expected in expected_profiles.items():
        ...
        observed = (int(profile["exhaustiveness"]), int(profile["max_step"]))
        if observed != expected:
            raise ValueError(f"official profile differs: {profile_id}")
    seeds = list(config["inputs"]["seeds"])
    if len(seeds) != 3 or len({str(row["seed_id"]) for row in seeds}) != 3:
        raise ValueError("exactly three unique seeds are required")
    expected = dict(config["expected"])
    if int(expected["validation_rows"]) != 0 or int(expected["test_rows"]) != 0:
...
def validate_inputs(
    ...
    if closure_ligands:
        raise ValueError("Stage 07 ligand inputs retain closure pseudoatoms")
...
def run(
    ...
    if len(all_rows) != expected_count:
        raise ValueError("complete Uni-Dock score count differs")
    unique_keys = {
        (
            row["profile_id"],
            row["seed_id"],
    ...
    if len(unique_keys) != len(all_rows):
        raise ValueError("complete Uni-Dock scores contain duplicate keys")
    ...
    result = {
        ...
        "experiment_id": config["experiment_id"],
        "status": "ok",
        "operation": "consumed Train-160 Uni-Dock profile sensitivity score generation",
        "config": {
            "path": relative_path(root, config_path),
