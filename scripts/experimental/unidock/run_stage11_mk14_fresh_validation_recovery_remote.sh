...
runner_script="scripts/experimental/unidock/run_stage11_mk14_fresh_validation_recovery_amendment01.py"
...
core_archive="$output_root/stage11_mk14_fresh_validation_unidock113_amendment01_core_v1.tar.gz"
diagnostic_archive="$output_root/stage11_mk14_fresh_validation_unidock113_amendment01_diagnostics_v1.tar.gz"
checkpoint_archive="$output_root/stage11_mk14_fresh_validation_unidock113_amendment01_checkpoint_v1.tar.gz"
failure_archive="$output_root/stage11_mk14_fresh_validation_unidock113_amendment01_failed_diagnostics_v1.tar.gz"

on_exit() {
  status=$?
  sync || true
  if [[ "$status" -ne 0 ]]; then
    failure_paths=("$config" "$amendment" "$runner_script")
