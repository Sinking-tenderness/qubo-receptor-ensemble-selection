...
run_dir="results/runs/stage61b_ppard_remaining144_unidock113_production"
progress="$run_dir/progress.json"
summary="$run_dir/summary.json"
audit="data/stage61b_ppard_remaining144_unidock113_production_audit.json"
...
stage60_result="data/stage60_ppard_transferred_qubo_freeze_result.json"
stage60_audit="data/stage60_ppard_transferred_qubo_freeze_audit.json"
stage60_model="data/stage60_ppard_transferred_qubo_model_record.json"
input_audit="data/stage61a_ppard_remaining144_unidock_input_audit.json"
ligand_manifest="data/processed/stage61a_ppard_remaining144_unidock_pdbqt_manifest.csv"
ligand_summary="data/stage61a_ppard_remaining144_unidock_input_summary.json"

partition="${PARTITION_ID:-${SEED_IDS:-all}}"
partition="${partition//,/+}"
partition="${partition//[^A-Za-z0-9_+-]/_}"
core_archive="$output_root/stage61b_ppard_remaining144_unidock113_production_core_v1.tar.gz"
diagnostic_archive="$output_root/stage61b_ppard_remaining144_unidock113_production_diagnostics_v1.tar.gz"
checkpoint_archive="$output_root/stage61b_ppard_remaining144_unidock113_${partition}_checkpoint_v1.tar.gz"
failure_archive="$output_root/stage61b_ppard_remaining144_unidock113_${partition}_failed_diagnostics_v1.tar.gz"
...
runner=(
  "$python_bin" -m scripts.experimental.unidock.run_stage61b_ppard_remaining144_production
  --config "$config"
)
"${runner[@]}" --audit-only
...
if [[ -f "$summary" ]]; then
  "$python_bin" -m scripts.experimental.unidock.audit_stage61b_ppard_remaining144_production \
    --config "$config"
  ...
  tar -czf "$core_archive" \
    ...
    "$receptor_summary" "$stage60_result" "$stage60_audit" "$stage60_model" \
    "$input_audit" "$ligand_manifest" "$ligand_summary" "$run_dir/environment" \
    "${core_batch_files[@]}"
  tar -czf "$diagnostic_archive" \
    "$config" "$audit" "$receptor_manifest" "$receptor_summary" \
    "$stage60_result" "$stage60_audit" "$stage60_model" \
    "$input_audit" "$ligand_manifest" "$ligand_summary" "$run_dir"
  sync
