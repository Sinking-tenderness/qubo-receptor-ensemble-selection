"""Run the frozen PPARG 24-receptor, three-seed cognate-redocking gate."""
...
def verify_implementation(root: Path, config: dict[str, object], key: str, expected: Path) -> None:
    ...
    if path.resolve() != expected.resolve():
        raise ValueError(f"Stage 18e implementation path differs: {key}")
    if common.file_sha256(path) != str(descriptor["sha256"]).upper():
        raise ValueError(f"Stage 18e implementation hash differs: {key}")
...
def validate_inputs(
    ...
    if any(int(value) != 0 for value in dict(config["data_boundary"]).values()):
        raise ValueError("Stage 18e crossed a frozen data boundary")
    ...
    if preparation.get("status") != "stage18d_pparg_redocking_inputs_ok":
        raise ValueError("Stage 18d PPARG input preparation did not pass")
    if any(int(value) != 0 for value in dict(preparation["data_boundary"]).values()):
        raise ValueError("Stage 18d preparation crossed a data boundary")
    for key in ("receptor_manifest_csv", "redocking_case_manifest_csv", "common_box_json"):
        ...
        if common.file_sha256(common.rooted_path(root, str(source["path"]))) != str(source["sha256"]).upper():
            raise ValueError(f"Stage 18d output identity differs: {key}")
    ...
    if receptor_ids != [str(value) for value in expected["receptor_ids"]]:
        raise ValueError("Stage 18e receptor order differs")
    if len(receptors) != int(expected["receptor_count"]) or len(cases) != int(expected["case_count"]):
        raise ValueError("Stage 18e receptor or case count differs")
    if [row["conformer_id"] for row in cases] != receptor_ids:
        raise ValueError("Stage 18e receptor and cognate-case order differs")
    if len({row["case_id"] for row in cases}) != len(cases):
        raise ValueError("Stage 18e case IDs are not unique")
    if any(row["status"] != "ok" for row in receptors + cases):
        raise ValueError("Stage 18e input manifest contains a failed row")
    for row in receptors:
        ...
        if not path.is_file() or common.file_sha256(path) != row["receptor_pdbqt_sha256"].upper():
            raise ValueError(f"Stage 18e receptor identity differs: {row['conformer_id']}")
    for row in cases:
        for path_key, hash_key in (("ligand_pdbqt", "ligand_pdbqt_sha256"), ("reference_sdf", "reference_sdf_sha256")):
            ...
            if not path.is_file() or common.file_sha256(path) != row[hash_key].upper():
                raise ValueError(f"Stage 18e case identity differs: {row['case_id']}")
        if common.macrocycle_closure_atom_types(common.rooted_path(root, row["ligand_pdbqt"])):
            raise ValueError(f"Stage 18e ligand retains closure pseudoatoms: {row['case_id']}")
    seeds = tuple((str(value["seed_id"]), int(value["base_seed"])) for value in inputs["seeds"])
    if seeds != EXPECTED_SEEDS:
        raise ValueError("Stage 18e seed ledger differs")
    if int(expected["redocking_pair_count"]) != len(receptors) * len(seeds):
        raise ValueError("Stage 18e redocking pair count differs")
...
def summarize_gate(
    ...
    for receptor_id in receptor_ids:
        ...
        if len(selected) != len(EXPECTED_SEEDS):
            raise ValueError(f"Stage 18e receptor result count differs: {receptor_id}")
...
def run(config_path: Path, root: Path, unidock: str | None, audit_only: bool, resume: bool) -> dict[str, object]:
    ...
    for case in cases:
        ...
        for seed in dict(config["inputs"])["seeds"]:
            ...
            ], f"PPARG redocking RMSD evaluation for {case['case_id']} {seed_id}")
            ...
            all_rows.append({
                "target_id": "PPARG", "case_id": case["case_id"],
    ...
    if len(all_rows) != expected_pairs or len({(row["conformer_id"], row["seed_id"]) for row in all_rows}) != expected_pairs:
        raise ValueError("Stage 18e complete redocking grid differs")
    ...
    status = "stage18e_pparg_cognate_redocking_gate_ok" if passed >= minimum_passing and unresolved == 0 and pose_failures == 0 else "stage18e_pparg_cognate_redocking_gate_failed"
    ...
    summary = {
        ...
        "failed_receptor_count": len(gate_rows) - passed,
        ...
        "next_gate": "select the frozen final 16 from passing PPARG receptors by the existing structural-distance rule" if status.endswith("_ok") else "stop PPARG benchmark docking and preserve the technical failure",
