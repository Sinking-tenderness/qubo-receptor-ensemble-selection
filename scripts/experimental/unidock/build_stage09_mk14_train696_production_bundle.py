"""Build the deterministic Stage 09 Train-696 Uni-Dock production bundle."""
...
try:
    ...
    from .run_stage09_mk14_train696_production import (
        file_sha256,
        read_json,
        validate_inputs,
    )
...
FIXED_PATHS = (
    CONFIG,
    "configs/stage09_mk14_train696_unidock_input_preparation.json",
    ...
    "scripts/experimental/unidock/run_stage09_mk14_train696_production_remote.sh",
    "reports/stage-09/mk14_train696_unidock113_production_execution.md",
)
...
def bundle_paths(root: Path) -> list[str]:
    ...
    if audit["validation_rows"] != 0 or audit["test_rows"] != 0:
        raise ValueError("Stage 09 bundle crossed a data boundary")
    if len(receptors) != 16 or len(ligands) != 696:
        raise ValueError("Stage 09 bundle input dimensions differ")
    if audit["expected_pair_count"] != 33408:
        raise ValueError("Stage 09 bundle pair count differs")
    ...
    for descriptor in config["implementation"].values():
        ...
        if file_sha256(root / path) != str(descriptor["sha256"]).upper():
            raise ValueError(f"Stage 09 implementation hash differs: {path}")
        paths.append(path)
    ...
    if any(
        ...
        raise ValueError("Stage 09 bundle contains a forbidden data path")
    return normalized
...
def main() -> int:
    ...
    result.update(
        {
            "operation": "Stage 09 Train-696 x expanded16 x three-seed Uni-Dock production bundle",
            "receptor_count": 16,
            "ligand_count": 696,
            "seed_count": 3,
            "gpu_batch_count": 48,
            "gpu_pair_count": 33408,
            "profile_id": "enhanced",
            "exhaustiveness": 1024,
            "max_step": 80,
            "rigid_macrocycle_ligand_count": 15,
            "validation_rows": 0,
            "test_rows": 0,
