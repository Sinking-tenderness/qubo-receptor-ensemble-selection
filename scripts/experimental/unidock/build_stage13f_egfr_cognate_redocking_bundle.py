"""Build the deterministic Stage 13f EGFR cognate-redocking GPU bundle."""
...
CONFIG = "configs/stage13f_egfr_cognate_redocking.json"
FIXED_PATHS = (
    CONFIG,
    "configs/stage13_multitarget_external_validation_master_preregistration.json",
    "configs/stage13e_egfr_redocking_input_preparation.json",
    "configs/stage13_egfr_external_target_pilot_preregistration.json",
    "configs/stage13_egfr_external_target_pilot_preregistration_amendment03.json",
    "configs/stage13_egfr_external_target_pilot_preregistration_amendment04.json",
    ...
    "scripts/experimental/unidock/run_stage13f_egfr_cognate_redocking.py",
    "scripts/experimental/unidock/run_stage13f_egfr_cognate_redocking_remote.sh",
    "scripts/experimental/unidock/build_stage13f_egfr_cognate_redocking_bundle.py",
    "reports/stage-13f/egfr_cognate_redocking_execution.md",
)
...
def bundle_paths(root: Path) -> list[str]:
    ...
    if audit["expected_redocking_pair_count"] != 48:
        raise ValueError("Stage 13f bundle pair count differs")
    if any(
        int(audit[key]) != 0
        for key in (
            "ligand_labels_read",
        ...
        raise ValueError("Stage 13f bundle crossed a data boundary")
    ...
    if any(marker in path.lower() for path in normalized for marker in forbidden):
        raise ValueError("Stage 13f bundle contains a forbidden result path")
    return normalized
...
def main() -> int:
    ...
    result.update(
        {
            "operation": "Stage 13f EGFR 16-receptor three-seed cognate redocking bundle",
            "target_id": "EGFR",
            "receptor_count": 16,
