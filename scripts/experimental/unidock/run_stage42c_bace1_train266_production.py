"""Run the frozen BACE1 Train-266 x 34 x 3 Uni-Dock production matrix."""
...
def verify_implementation(
    ...
    if path.resolve() != expected_path.resolve():
        raise ValueError(f"Stage 42c implementation path differs: {key}")
    if common.file_sha256(path) != str(descriptor["sha256"]).upper():
        raise ValueError(f"Stage 42c implementation hash differs: {key}")
...
def validate_config(config: dict[str, object]) -> None:
    ...
    for key, value in fixed_counts.items():
        if int(expected[key]) != value:
            raise ValueError(f"Stage 42c expected count differs: {key}")
    ...
    if profile != FROZEN_PROFILE:
        raise ValueError("Stage 42c frozen Uni-Dock profile differs")
    if str(protocol["required_package_version"]) != "1.1.3" or int(protocol["num_modes"]) != 1:
        raise ValueError("Stage 42c Uni-Dock version or pose count differs")
...
def validate_inputs(
    ...
    if preregistration.get("preregistration_id") != "stage42-bace1-redocking-qualified-development-20260804-v1":
        raise ValueError("the Stage 42 BACE1 development experiment was not frozen")
    if input_audit.get("status") != "independent_stage42b_bace1_train266_input_audit_ok":
        raise ValueError("the independent Stage 42b input audit did not pass")
    if preparation.get("status") != "stage42b_bace1_train266_unidock_inputs_ok":
        raise ValueError("the Stage 42b ligand preparation did not pass")
    if int(preparation["data_boundary"]["fresh_validation_rows_read"]) != 0 or int(
        preparation["data_boundary"]["test_rows_read"]
    ) != 0:
        raise ValueError("Stage 42b preparation exposed protected rows")
    ...
    if receptor_ids != [str(value) for value in expected["receptor_ids"]]:
        raise ValueError("Stage 42c receptor order differs")
    if receptor_ids != [str(value) for value in preregistration["receptor_pool"]["receptor_ids"]]:
        raise ValueError("Stage 42c receptors differ from the frozen qualified pool")
    if len(receptors) != int(expected["receptor_count"]) or any(row["status"] != "ok" for row in receptors):
        raise ValueError("Stage 42c receptor manifest differs")
    if len(ligands) != int(expected["ligand_count"]):
        raise ValueError("Stage 42c ligand count differs")
    if len({row["ligand_id"] for row in ligands}) != len(ligands):
        raise ValueError("Stage 42c ligand IDs are not unique")
    ...
    if labels != expected_labels:
        raise ValueError("Stage 42c ligand labels differ")
    if {row["split"] for row in ligands} != {boundary["allowed_split"]}:
        raise ValueError("Stage 42c exposed a non-train ligand")
    if {row["selection_role"] for row in ligands} != {boundary["allowed_selection_role"]}:
        raise ValueError("Stage 42c ligand selection role differs")
    if any(row["pdbqt_status"] != "ok" for row in ligands):
        raise ValueError("Stage 42c ligand manifest contains a failed PDBQT")
    for rows, path_column, hash_column, id_column in (
        ...
        for row in rows:
            ...
            if not path.is_file() or common.file_sha256(path) != row[hash_column].upper():
                raise ValueError(f"Stage 42c prepared input differs: {row[id_column]}")
    pseudoatom_ids = [
        row["ligand_id"]
        for row in ligands
    ...
    if pseudoatom_ids:
        raise ValueError(f"Stage 42c ligands retain closure pseudoatoms: {pseudoatom_ids}")
    ...
    if variants != Counter({key: int(value) for key, value in dict(preparation["preparation_variant_counts"]).items()}):
        raise ValueError("Stage 42c preparation variants differ")
...
def finalize(
    ...
    progress = {
        ...
        "status": "stage42c_production_complete" if not missing else "stage42c_partial_ok",
    ...
    if len(rows) != expected_pairs:
        raise ValueError("complete Stage 42c pair count differs")
    unique = {(str(row["seed_id"]), str(row["receptor_id"]), str(row["ligand_id"])) for row in rows}
    if len(unique) != expected_pairs:
        raise ValueError("complete Stage 42c keys are not unique")
    ...
    for row in rows:
        ...
        if not math.isfinite(score) or abs(score) > float(config["unidock"]["maximum_absolute_score_kcal_per_mol"]):
            raise ValueError(f"invalid Stage 42c score: {row['ligand_id']}")
    ...
    result = {
        "schema_version": "1.0",
        "experiment_id": config["experiment_id"],
        "status": "stage42c_bace1_train266_unidock_matrix_ok",
        "operation": "outcome-informed post-hoc development-only Uni-Dock production score generation",
        "experiment_class": "outcome_informed_posthoc_development",
        "stage41c_gate_status": "closed_failed_34_of_49_below_40_required",
        ...
        "next_gate": "run the independent Stage 42c matrix audit before any train-only QUBO or comparator analysis",
        ...
        for seed in selected_seeds:
            ...
            for receptor in selected_receptors:
                ...
                if summary["status"] != "ok":
                    raise ValueError(f"Stage 42c technical gate failed: {seed_id}/{receptor_id}")
                executed_batches += 1
...
def main() -> int:
    ...
    return 0 if result["status"] in {
        "audit_only_ok", "stage42c_partial_ok", "stage42c_bace1_train266_unidock_matrix_ok"
    } else 2
