"""Independently audit the complete Stage61b Remaining-144 matrix."""
...
def run_audit(config_path: Path, root: Path) -> dict[str, object]:
    ...
    if auditor_path.resolve() != Path(__file__).resolve():
        raise ValueError("Stage61b independent auditor path differs")
    if runner.common.file_sha256(auditor_path) != str(descriptor["sha256"]).upper():
        raise ValueError("Stage61b independent auditor hash differs")
    ...
    if summary.get("status") != "stage61b_ppard_remaining144_unidock_matrix_ok":
        raise ValueError("Stage61b source matrix did not pass")
    if str(summary["config"]["sha256"]).upper() != runner.common.file_sha256(config_path):
        raise ValueError("Stage61b source config hash differs")
    if any(int(value) != 0 for value in dict(summary["data_boundary"]).values()):
        raise ValueError("Stage61b source crossed a data boundary")
    ...
    for seed in dict(config["inputs"])["seeds"]:
        ...
        for receptor in receptors:
            ...
            if batch_summary.get("status") != "ok":
                raise ValueError(f"Stage61b batch failed: {seed_id}/{receptor_id}")
            ...
            if batch_summary.get("signature") != signature:
                raise ValueError(f"Stage61b signature differs: {seed_id}/{receptor_id}")
            if runner.common.file_sha256(paths["scores"]) != str(
                batch_summary["scores_sha256"]
            ).upper():
                raise ValueError(f"Stage61b score hash differs: {seed_id}/{receptor_id}")
            if runner.common.file_sha256(paths["log"]) != str(
                batch_summary["log_sha256"]
            ).upper():
                raise ValueError(f"Stage61b log hash differs: {seed_id}/{receptor_id}")
            if len(rows) != len(ligands) or {row["ligand_id"] for row in rows} != ligand_ids:
                raise ValueError(f"Stage61b batch ligand grid differs: {seed_id}/{receptor_id}")
            ...
            if pose_audit != dict(batch_summary["pose_integrity_audit"]):
                raise ValueError(f"Stage61b pose audit differs: {seed_id}/{receptor_id}")
            warning = runner.common.classify_warning_log(paths["log"], pose_audit)
            if warning != dict(batch_summary["warning_adjudication"]):
                raise ValueError(f"Stage61b warning audit differs: {seed_id}/{receptor_id}")
            if int(warning["unresolved_warning_event_count"]) != 0:
                raise ValueError(f"Stage61b unresolved warning: {seed_id}/{receptor_id}")
            known_warnings += int(warning["known_warning_event_count"])
            for source, audited in zip(rows, audited_rows, strict=True):
                pose = runner.common.rooted_path(root, source["output_pose_path"])
                if runner.common.file_sha256(pose) != source["output_pose_sha256"].upper():
                    raise ValueError(f"Stage61b pose hash differs: {source['ligand_id']}")
                score, pose_count = prior.parse_vina_pose(pose)
                if not math.isfinite(score) or abs(score - float(source["gpu_score"])) > 1e-9:
                    raise ValueError(f"Stage61b pose score differs: {source['ligand_id']}")
                if pose_count != 1 or int(source["pose_count"]) != 1:
                    raise ValueError(f"Stage61b pose count differs: {source['ligand_id']}")
                if audited["pose_integrity_status"] != "ok":
                    raise ValueError(f"Stage61b pose integrity failed: {source['ligand_id']}")
    ...
    if batch_count != int(expected["batch_count"]) or len(recomputed_rows) != int(
        ...
        raise ValueError("Stage61b independent matrix dimensions differ")
    ...
    if len(batch_rows) != batch_count or any(
        row["status"] != "ok"
        or int(row["unresolved_warning_event_count"]) != 0
        or int(row["pose_integrity_failure_count"]) != 0
        for row in batch_rows
    ):
        raise ValueError("Stage61b batch ledger technical gate differs")
    ...
    if int(summary["known_warning_event_count"]) != known_warnings:
        raise ValueError("Stage61b known-warning total differs")
    if int(summary["unresolved_warning_event_count"]) != 0 or int(
        summary["pose_integrity_failure_count"]
    ) != 0:
        raise ValueError("Stage61b source technical totals differ")

    result = {
        "schema_version": "1.0",
        "audit_id": "stage61b-ppard-remaining144-unidock113-independent-matrix-audit-v1",
        "status": "independent_stage61b_ppard_remaining144_unidock_matrix_audit_ok",
        ...
        "next_gate": "merge with Stage58b Pilot-96 and run the frozen Stage60 nested QUBO analysis",
        "decision_boundary": (
            "This audit establishes only the PPARD Remaining-144 score matrix. "
            "It does not establish QUBO efficacy, independent validation, quantum "
            "execution, speedup, or quantum advantage."
        ),
    }
