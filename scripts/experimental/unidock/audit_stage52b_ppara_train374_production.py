"""Independently audit the complete Stage 52b PPARA Uni-Dock matrix."""
...
from scripts.experimental.unidock import run_stage52b_ppara_train374_production as runner
...
def run_audit(config_path: Path, root: Path) -> dict[str, object]:
    ...
    if auditor_path.resolve() != Path(__file__).resolve():
        raise ValueError("Stage 52b independent auditor path differs")
    if runner.common.file_sha256(auditor_path) != str(descriptor["sha256"]).upper():
        raise ValueError("Stage 52b independent auditor hash differs")

    outputs = dict(config["outputs"])
    summary_path = runner.common.rooted_path(root, str(outputs["summary_json"]))
    summary = runner.common.read_json(summary_path)
    if summary.get("status") != "stage52b_ppara_train374_unidock_matrix_ok":
        raise ValueError("Stage 52b source matrix did not pass")
    if str(summary["config"]["sha256"]).upper() != runner.common.file_sha256(
        config_path
    ):
        raise ValueError("Stage 52b source config hash differs")
    if any(int(value) != 0 for value in dict(summary["data_boundary"]).values()):
        raise ValueError("Stage 52b source crossed a data boundary")
    ...
    for seed in dict(config["inputs"])["seeds"]:
        ...
        for receptor in receptors:
            ...
            if batch_summary.get("status") != "ok":
                raise ValueError(f"Stage 52b batch failed: {seed_id}/{receptor_id}")
            ...
            if batch_summary.get("signature") != signature:
                raise ValueError(f"Stage 52b signature differs: {seed_id}/{receptor_id}")
            if runner.common.file_sha256(paths["scores"]) != str(
                batch_summary["scores_sha256"]
            ).upper():
                raise ValueError(f"Stage 52b score hash differs: {seed_id}/{receptor_id}")
            if runner.common.file_sha256(paths["log"]) != str(
                batch_summary["log_sha256"]
            ).upper():
                raise ValueError(f"Stage 52b log hash differs: {seed_id}/{receptor_id}")
            if len(rows) != len(ligands) or {
                row["ligand_id"] for row in rows
            } != ligand_ids:
                raise ValueError(f"Stage 52b batch ligand grid differs: {seed_id}/{receptor_id}")
            audited_rows, pose_audit = runner.common.audit_batch_poses(
                root, ligands, rows
            )
            if pose_audit != dict(batch_summary["pose_integrity_audit"]):
                raise ValueError(f"Stage 52b pose audit differs: {seed_id}/{receptor_id}")
            warning = runner.common.classify_warning_log(paths["log"], pose_audit)
            if warning != dict(batch_summary["warning_adjudication"]):
                raise ValueError(f"Stage 52b warning audit differs: {seed_id}/{receptor_id}")
            if int(warning["unresolved_warning_event_count"]) != 0:
                raise ValueError(f"Stage 52b unresolved warning: {seed_id}/{receptor_id}")
            ...
            for source, audited in zip(rows, audited_rows, strict=True):
                ...
                if runner.common.file_sha256(output) != source[
                    "output_pose_sha256"
                ].upper():
                    raise ValueError(f"Stage 52b pose hash differs: {source['ligand_id']}")
                score, pose_count = prior.parse_vina_pose(output)
                if not math.isfinite(score) or abs(
                    score - float(source["gpu_score"])
                ) > 1e-9:
                    raise ValueError(f"Stage 52b pose score differs: {source['ligand_id']}")
                if pose_count != 1 or int(source["pose_count"]) != 1:
                    raise ValueError(f"Stage 52b pose count differs: {source['ligand_id']}")
                if audited["pose_integrity_status"] != "ok":
                    raise ValueError(
                        f"Stage 52b pose integrity failed: {source['ligand_id']}"
                    )
    ...
    if batch_count != int(expected["batch_count"]) or len(recomputed_rows) != int(
        ...
        raise ValueError("Stage 52b independent matrix dimensions differ")
    ...
    if len(batch_rows) != batch_count:
        raise ValueError("Stage 52b batch ledger count differs")
    if any(
        row["status"] != "ok"
        ...
        raise ValueError("Stage 52b batch ledger technical gate differs")
    prior.compare_matrix(
        runner.common.read_csv(median_path),
        aggregate_rows,
        ligands,
        receptor_ids,
        "median",
    ...
    prior.compare_matrix(
        runner.common.read_csv(minimum_path),
        aggregate_rows,
        ligands,
        receptor_ids,
        "minimum",
    ...
    if int(summary["known_warning_event_count"]) != known_warnings:
        raise ValueError("Stage 52b known-warning total differs")
    if int(summary["unresolved_warning_event_count"]) != 0 or int(
        summary["pose_integrity_failure_count"]
    ) != 0:
        raise ValueError("Stage 52b source technical totals differ")

    result = {
        "schema_version": "1.0",
        "audit_id": "stage52b-ppara-train374-unidock113-independent-matrix-audit-v1",
        "status": "independent_stage52b_ppara_train374_unidock_matrix_audit_ok",
        ...
        "next_gate": (
            "perform the frozen train-only PPARA large-pool QUBO, greedy, exact, "
            "and literature-baseline analysis"
        ),
        "decision_boundary": (
            "This audit establishes only the post-hoc PPARA Train-374 development "
            "matrix. It cannot rescue Stage51 or establish independent validation, "
            "QUBO benefit, quantum speedup, or quantum advantage."
        ),
    }
