"""Run the frozen BACE1 large-pool, three-seed cognate-redocking gate."""
...
def verify_implementation(root: Path, config: dict[str, object], key: str, expected: Path) -> None:
    ...
    if path.resolve() != expected.resolve():
        raise ValueError(f"Stage 41c implementation path differs: {key}")
    if common.file_sha256(path) != str(descriptor["sha256"]).upper():
        raise ValueError(f"Stage 41c implementation hash differs: {key}")
...
def validate_inputs(
    ...
    if any(int(value) != 0 for value in dict(config["data_boundary"]).values()):
        raise ValueError("Stage 41c crossed a frozen data boundary")
    ...
    if preparation.get("status") != "stage41b_bace1_large_pool_redocking_inputs_partial_gate_ready":
        raise ValueError("Stage 41b BACE1 input preparation did not pass its partial gate")
    if preparation.get("technical_gate_ready") is not True:
        raise ValueError("Stage 41b BACE1 technical gate is not ready")
    if any(int(value) != 0 for value in dict(preparation["data_boundary"]).values()):
        raise ValueError("Stage 41b preparation crossed a data boundary")
    for key in ("receptor_manifest_csv", "redocking_case_manifest_csv", "common_box_json"):
        ...
        if common.file_sha256(common.rooted_path(root, str(source["path"]))) != str(source["sha256"]).upper():
            raise ValueError(f"Stage 41b output identity differs: {key}")
    ...
    if receptor_ids != [str(value) for value in expected["receptor_ids"]]:
        raise ValueError("Stage 41c receptor order differs")
    if len(receptors) != int(expected["receptor_count"]) or len(cases) != int(expected["case_count"]):
        raise ValueError("Stage 41c receptor or case count differs")
    if [row["conformer_id"] for row in cases] != receptor_ids:
        raise ValueError("Stage 41c receptor and cognate-case order differs")
    if len({row["case_id"] for row in cases}) != len(cases):
        raise ValueError("Stage 41c case IDs are not unique")
    if any(row["status"] != "ok" for row in receptors + cases):
        raise ValueError("Stage 41c input manifest contains a failed row")
    for row in receptors:
        ...
        if not path.is_file() or common.file_sha256(path) != row["receptor_pdbqt_sha256"].upper():
            raise ValueError(f"Stage 41c receptor identity differs: {row['conformer_id']}")
    for row in cases:
        for path_key, hash_key in (("ligand_pdbqt", "ligand_pdbqt_sha256"), ("reference_sdf", "reference_sdf_sha256")):
            ...
            if not path.is_file() or common.file_sha256(path) != row[hash_key].upper():
                raise ValueError(f"Stage 41c case identity differs: {row['case_id']}")
        if common.macrocycle_closure_atom_types(common.rooted_path(root, row["ligand_pdbqt"])):
            raise ValueError(f"Stage 41c ligand retains closure pseudoatoms: {row['case_id']}")
    seeds = tuple((str(value["seed_id"]), int(value["base_seed"])) for value in inputs["seeds"])
    if seeds != EXPECTED_SEEDS:
        raise ValueError("Stage 41c seed ledger differs")
    if int(expected["redocking_pair_count"]) != len(receptors) * len(seeds):
        raise ValueError("Stage 41c redocking pair count differs")
    ...
    if frozen_count != len(receptors) + len(preparation_failures):
        raise ValueError("Stage 41c frozen-pool accounting differs")
    ...
    if observed_failure_ids != expected_failure_ids:
        raise ValueError("Stage 41c preparation-failure ledger differs")
...
def summarize_gate(
    ...
    for receptor_id in receptor_ids:
        ...
        if len(selected) != len(EXPECTED_SEEDS):
            raise ValueError(f"Stage 41c receptor result count differs: {receptor_id}")
...
def run(config_path: Path, root: Path, unidock: str | None, audit_only: bool, resume: bool) -> dict[str, object]:
    ...
    for case in cases:
        ...
        for seed in dict(config["inputs"])["seeds"]:
            ...
            ], f"BACE1 redocking RMSD evaluation for {case['case_id']} {seed_id}")
            ...
            all_rows.append({
                "target_id": "BACE1", "case_id": case["case_id"],
    ...
    if len(all_rows) != expected_pairs or len({(row["conformer_id"], row["seed_id"]) for row in all_rows}) != expected_pairs:
        raise ValueError("Stage 41c complete redocking grid differs")
    ...
    for failure in input_audit["technical_preparation_failures"]:
        gate_rows.append({
            ...
            "gate_pass": False,
            "failure_stage": "stage41b_input_preparation",
    ...
    status = "stage41c_bace1_large_pool_cognate_redocking_gate_ok" if passed >= minimum_passing and unresolved == 0 and pose_failures == 0 else "stage41c_bace1_large_pool_cognate_redocking_gate_failed"
    ...
    summary = {
        ...
        "next_gate": "prepare the frozen 266-ligand BACE1 development panel and dock it against every passing receptor without structural max-min compression" if status.endswith("_ok") else "stop the BACE1 large-pool route and preserve all technical failures",
