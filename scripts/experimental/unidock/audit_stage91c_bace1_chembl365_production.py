"""Independently audit the complete Stage91c BACE1 Uni-Dock matrix."""
...
from scripts.experimental.unidock import run_stage91c_bace1_chembl365_production as runner
...
def run_audit(config_path: Path, root: Path) -> dict[str, object]:
    ...
    if auditor_path.resolve() != Path(__file__).resolve():
        raise ValueError("Stage91c independent auditor path differs")
    if runner.common.file_sha256(auditor_path) != str(descriptor["sha256"]).upper():
        raise ValueError("Stage91c independent auditor hash differs")
    ...
    if summary.get("status") != "stage91c_bace1_chembl365_unidock_matrix_ok":
        raise ValueError("Stage91c source matrix did not pass")
    if str(summary["config"]["sha256"]).upper() != runner.common.file_sha256(
        config_path
    ):
        raise ValueError("Stage91c source config hash differs")
    if any(int(value) != 0 for value in dict(summary["data_boundary"]).values()):
        raise ValueError("Stage91c source crossed a data boundary")
    ...
    for seed in dict(config["inputs"])["seeds"]:
        ...
        for receptor in receptors:
            ...
            if batch_summary.get("status") != "ok":
                raise ValueError(f"Stage91c batch failed: {seed_id}/{receptor_id}")
            ...
            if batch_summary.get("signature") != signature:
                raise ValueError(f"Stage91c signature differs: {seed_id}/{receptor_id}")
            if runner.common.file_sha256(paths["scores"]) != str(
                batch_summary["scores_sha256"]
            ).upper():
                raise ValueError(f"Stage91c score hash differs: {seed_id}/{receptor_id}")
            if runner.common.file_sha256(paths["log"]) != str(
                batch_summary["log_sha256"]
            ).upper():
                raise ValueError(f"Stage91c log hash differs: {seed_id}/{receptor_id}")
            if len(rows) != len(ligands) or {row["ligand_id"] for row in rows} != ligand_ids:
                raise ValueError(f"Stage91c batch ligand grid differs: {seed_id}/{receptor_id}")
            ...
            if pose_audit != dict(batch_summary["pose_integrity_audit"]):
                raise ValueError(f"Stage91c pose audit differs: {seed_id}/{receptor_id}")
            warning = runner.common.classify_warning_log(paths["log"], pose_audit)
            if warning != dict(batch_summary["warning_adjudication"]):
                raise ValueError(f"Stage91c warning audit differs: {seed_id}/{receptor_id}")
            if int(warning["unresolved_warning_event_count"]) != 0:
                raise ValueError(f"Stage91c unresolved warning: {seed_id}/{receptor_id}")
            ...
            for source, audited in zip(rows, audited_rows, strict=True):
                ...
                if runner.common.file_sha256(output) != source["output_pose_sha256"].upper():
                    raise ValueError(f"Stage91c pose hash differs: {source['ligand_id']}")
                score, pose_count = prior.parse_vina_pose(output)
                if not math.isfinite(score) or abs(score - float(source["gpu_score"])) > 1e-9:
                    raise ValueError(f"Stage91c pose score differs: {source['ligand_id']}")
                if pose_count != 1 or int(source["pose_count"]) != 1:
                    raise ValueError(f"Stage91c pose count differs: {source['ligand_id']}")
                if audited["pose_integrity_status"] != "ok":
                    raise ValueError(f"Stage91c pose integrity failed: {source['ligand_id']}")
            recomputed_rows.extend(rows)
            batch_count += 1

    if batch_count != 102 or len(recomputed_rows) != 37230:
        raise ValueError("Stage91c independent matrix dimensions differ")
    ...
    if len(batch_rows) != 102:
        raise ValueError("Stage91c batch ledger count differs")
    if any(
        row["status"] != "ok"
        ...
        raise ValueError("Stage91c batch ledger technical gate differs")
    ...
    if int(summary["known_warning_event_count"]) != known_warnings:
        raise ValueError("Stage91c known-warning total differs")
    if int(summary["unresolved_warning_event_count"]) != 0 or int(
        summary["pose_integrity_failure_count"]
    ) != 0:
        raise ValueError("Stage91c source technical totals differ")

    result = {
        "schema_version": "1.0",
        "audit_id": "stage91c-bace1-chembl365-unidock113-independent-matrix-audit-v1",
        "status": "independent_stage91c_bace1_chembl365_unidock_matrix_audit_ok",
        ...
        "next_gate": "run the frozen Stage91 objective, greedy, exact, local-search, and hardness analyses",
