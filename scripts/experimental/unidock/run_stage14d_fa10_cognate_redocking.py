"""Run and adjudicate the frozen FA10 16-receptor cognate-redocking gate."""
...
def verify_implementation(
    ...
    if path.resolve() != expected.resolve():
        raise ValueError(f"Stage 14d implementation path differs: {key}")
    if file_sha256(path) != str(descriptor["sha256"]).upper():
        raise ValueError(f"Stage 14d implementation hash differs: {key}")
...
def runtime_evidence(config: dict[str, object]) -> dict[str, str]:
    ...
    if actual != expected:
        raise RuntimeError(f"Stage 14d runtime differs: {actual} != {expected}")
    return actual
...
def validate_protocol(
    ...
    for key, expected in EXPECTED_PROTOCOL.items():
        ...
        if observed != expected:
            raise ValueError(f"Stage 14d frozen protocol differs: {key}")
    ...
    if {key: float(box[key]) for key in expected_box} != expected_box:
        raise ValueError("Stage 14d Uni-Dock box differs from Stage 14c")
...
def validate_inputs(
    ...
    if any(int(value) != 0 for value in boundary.values()):
        raise ValueError("Stage 14d crossed a frozen data boundary")
    ...
    if preparation.get("status") != "stage14c_fa10_redocking_inputs_ok":
        raise ValueError("Stage 14c input preparation did not pass")
    if any(int(value) != 0 for value in dict(preparation["data_boundary"]).values()):
        raise ValueError("Stage 14c preparation crossed a data boundary")
    for key in ("receptor_manifest_csv", "redocking_case_manifest_csv", "common_box_json"):
        ...
        if file_sha256(rooted_path(root, str(source["path"]))) != str(
            ...
            raise ValueError(f"Stage 14c output identity differs: {key}")
    ...
    if receptor_ids != [str(value) for value in expected["receptor_ids"]]:
        raise ValueError("Stage 14d receptor order differs")
    if len(receptors) != int(expected["receptor_count"]):
        raise ValueError("Stage 14d receptor count differs")
    if len(cases) != int(expected["case_count"]):
        raise ValueError("Stage 14d case count differs")
    if [row["conformer_id"] for row in cases] != receptor_ids:
        raise ValueError("Stage 14d receptor and cognate-case order differs")
    if len({row["case_id"] for row in cases}) != len(cases):
        raise ValueError("Stage 14d case IDs are not unique")
    if any(row["status"] != "ok" for row in receptors + cases):
        raise ValueError("Stage 14d input manifest contains a failed row")
    ...
    for row in receptors:
        ...
        if not path.is_file() or file_sha256(path) != row[
            ...
            raise ValueError(f"Stage 14d receptor identity differs: {row['conformer_id']}")
    for row in cases:
        for path_key, hash_key in (
            ...
            if not path.is_file() or file_sha256(path) != row[hash_key].upper():
                raise ValueError(f"Stage 14d case identity differs: {row['case_id']}")
        pseudoatoms = macrocycle_closure_atom_types(
            rooted_path(root, row["ligand_pdbqt"])
        )
        if pseudoatoms:
            raise ValueError(
                f"Stage 14d ligand retains closure pseudoatoms: {row['case_id']}"
            )
    ...
    if seeds != EXPECTED_SEEDS:
        raise ValueError("Stage 14d seed ledger differs")
    if int(expected["redocking_pair_count"]) != len(receptors) * len(seeds):
        raise ValueError("Stage 14d redocking pair count differs")
    ...
    return receptors, cases, {
        ...
        "mechanism_diagnostic": "all selected complexes passed the frozen noncovalent coordinate gate",
...
def summarize_gate(
    ...
    for receptor_id in receptor_ids:
        ...
        if len(selected) != len(EXPECTED_SEEDS):
            raise ValueError(f"Stage 14d receptor result count differs: {receptor_id}")
...
def run(
    ...
    for case in cases:
        ...
        for seed in dict(config["inputs"])["seeds"]:
            ...
            run_checked(
                ...
                f"FA10 redocking RMSD evaluation for {case['case_id']} {seed_id}",
            )
    ...
    if len(all_rows) != expected_pairs or len(
        ...
        raise ValueError("Stage 14d complete redocking grid differs")
    ...
    status = (
        "stage14d_fa10_cognate_redocking_gate_ok"
        if all_pass and unresolved == 0 and pose_failures == 0
        else "stage14d_fa10_cognate_redocking_gate_failed"
    )
    ...
    summary = {
        ...
        "next_gate": (
            "prepare the untouched FA10 Train-696 ligand panel and run 696 x 16 x 3 Uni-Dock production"
            if status == "stage14d_fa10_cognate_redocking_gate_ok"
            else "stop FA10 benchmark docking, preserve the technical failure, and continue to HIVPR"
        ),
