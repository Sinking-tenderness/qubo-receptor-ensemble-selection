"""Independently audit the complete Stage 42c BACE1 Uni-Dock matrix."""
...
from scripts.experimental.unidock import run_stage42c_bace1_train266_production as runner
...
def run_audit(config_path: Path, root: Path) -> dict[str, object]:
    ...
    if auditor_path.resolve() != Path(__file__).resolve():
        raise ValueError("Stage 42c independent auditor path differs")
    if runner.common.file_sha256(auditor_path) != str(descriptor["sha256"]).upper():
        raise ValueError("Stage 42c independent auditor hash differs")

    outputs = dict(config["outputs"])
    summary_path = runner.common.rooted_path(root, str(outputs["summary_json"]))
    summary = runner.common.read_json(summary_path)
    if summary.get("status") != "stage42c_bace1_train266_unidock_matrix_ok":
        raise ValueError("Stage 42c source matrix did not pass")
    if str(summary["config"]["sha256"]).upper() != runner.common.file_sha256(config_path):
        raise ValueError("Stage 42c source config hash differs")
    if any(int(value) != 0 for value in dict(summary["data_boundary"]).values()):
        raise ValueError("Stage 42c source crossed a data boundary")
    ...
    for seed in dict(config["inputs"])["seeds"]:
        ...
        for receptor in receptors:
            ...
            if batch_summary.get("status") != "ok":
                raise ValueError(f"Stage 42c batch failed: {seed_id}/{receptor_id}")
            ...
            if batch_summary.get("signature") != signature:
                raise ValueError(f"Stage 42c signature differs: {seed_id}/{receptor_id}")
            if runner.common.file_sha256(paths["scores"]) != str(batch_summary["scores_sha256"]).upper():
                raise ValueError(f"Stage 42c score hash differs: {seed_id}/{receptor_id}")
            if runner.common.file_sha256(paths["log"]) != str(batch_summary["log_sha256"]).upper():
                raise ValueError(f"Stage 42c log hash differs: {seed_id}/{receptor_id}")
            if len(rows) != len(ligands) or {row["ligand_id"] for row in rows} != ligand_ids:
                raise ValueError(f"Stage 42c batch ligand grid differs: {seed_id}/{receptor_id}")
            ...
            if pose_audit != dict(batch_summary["pose_integrity_audit"]):
                raise ValueError(f"Stage 42c pose audit differs: {seed_id}/{receptor_id}")
            warning = runner.common.classify_warning_log(paths["log"], pose_audit)
            if warning != dict(batch_summary["warning_adjudication"]):
                raise ValueError(f"Stage 42c warning audit differs: {seed_id}/{receptor_id}")
            if int(warning["unresolved_warning_event_count"]) != 0:
                raise ValueError(f"Stage 42c unresolved warning: {seed_id}/{receptor_id}")
            ...
            for source, audited in zip(rows, audited_rows, strict=True):
                ...
                if runner.common.file_sha256(output) != source["output_pose_sha256"].upper():
                    raise ValueError(f"Stage 42c pose hash differs: {source['ligand_id']}")
                score, pose_count = prior.parse_vina_pose(output)
                if not math.isfinite(score) or abs(score - float(source["gpu_score"])) > 1e-9:
                    raise ValueError(f"Stage 42c pose score differs: {source['ligand_id']}")
                if pose_count != 1 or int(source["pose_count"]) != 1:
                    raise ValueError(f"Stage 42c pose count differs: {source['ligand_id']}")
                if audited["pose_integrity_status"] != "ok":
                    raise ValueError(f"Stage 42c pose integrity failed: {source['ligand_id']}")
            recomputed_rows.extend(rows)
            batch_count += 1

    if batch_count != int(expected["batch_count"]) or len(recomputed_rows) != int(expected["pair_count"]):
        raise ValueError("Stage 42c independent matrix dimensions differ")
    ...
    if len(batch_rows) != batch_count:
        raise ValueError("Stage 42c batch ledger count differs")
    if any(
        row["status"] != "ok"
        ...
        raise ValueError("Stage 42c batch ledger technical gate differs")
    ...
    if int(summary["known_warning_event_count"]) != known_warnings:
        raise ValueError("Stage 42c known-warning total differs")
    if int(summary["unresolved_warning_event_count"]) != 0 or int(summary["pose_integrity_failure_count"]) != 0:
        raise ValueError("Stage 42c source technical totals differ")

    result = {
        "schema_version": "1.0",
        "audit_id": "stage42c-bace1-train266-unidock113-independent-matrix-audit-v1",
        "status": "independent_stage42c_bace1_train266_unidock_matrix_audit_ok",
        ...
        "decision_boundary": "This audit establishes only the outcome-informed BACE1 Train-266 development matrix. It cannot rescue Stage 41c and does not establish independent validation performance, QUBO benefit, or quantum advantage.",
    }
