...
run_dir="results/runs/stage19c_pparg_train668_unidock113_production"
progress="$run_dir/progress.json"
summary="$run_dir/summary.json"
audit="data/stage19c_pparg_train668_unidock113_production_audit.json"
...
receptor_manifest="data/processed/stage18i_pparg_exploratory_final16_prepared_receptor_manifest.csv"
selection_summary="data/stage18i_pparg_exploratory_final16_selection_summary.json"
ligand_manifest="data/processed/stage19b_pparg_train668_unidock_pdbqt_manifest.csv"
ligand_summary="data/stage19b_pparg_train668_unidock_input_summary.json"

partition="${PARTITION_ID:-${SEED_IDS:-all}}"
partition="${partition//,/+}"
partition="${partition//[^A-Za-z0-9_+-]/_}"
core_archive="$output_root/stage19c_pparg_train668_unidock113_production_core_v1.tar.gz"
diagnostic_archive="$output_root/stage19c_pparg_train668_unidock113_production_diagnostics_v1.tar.gz"
checkpoint_archive="$output_root/stage19c_pparg_train668_unidock113_${partition}_checkpoint_v1.tar.gz"
failure_archive="$output_root/stage19c_pparg_train668_unidock113_${partition}_failed_diagnostics_v1.tar.gz"
...
if [[ -f "$summary" ]]; then
  "$python_bin" -m scripts.experimental.unidock.audit_stage19c_pparg_train668_production \
    --config "$config"
  ...
  tar -czf "$core_archive" \
    ...
    "$selection_summary" "$ligand_manifest" "$ligand_summary" \
    "$run_dir/environment" "${core_batch_files[@]}"

  tar -czf "$diagnostic_archive" \
    "$config" "$audit" "$receptor_manifest" "$selection_summary" \
