"""Build the deterministic Stage 07 Uni-Dock sensitivity remote bundle."""
...
FIXED_PATHS = (
    ...
    "reports/stage-07/mk14_unidock113_train160_search_sensitivity_execution.md",
)
...
def bundle_paths(root: Path) -> list[str]:
    ...
    if audit["validation_rows"] != 0 or audit["test_rows"] != 0:
        raise ValueError("Stage 07 sensitivity bundle crossed a data boundary")
    if audit["macrocycle_closure_pseudoatom_ligand_count"] != 0:
        raise ValueError("Stage 07 sensitivity bundle contains closure pseudoatoms")
...
def main() -> int:
    ...
    result.update(
        {
            "operation": "Stage 07 consumed Train-160 Uni-Dock official-profile sensitivity bundle",
            "profile_ids": ["fast", "balance", "detail"],
            "receptor_count": 4,
            "ligand_count": 160,
            "seed_count": 3,
            "gpu_pair_count": 5760,
