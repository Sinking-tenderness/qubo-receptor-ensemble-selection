"""Independently audit the complete Stage58b PPARD Pilot-96 matrix."""
...
def run_audit(config_path: Path, root: Path) -> dict[str, object]:
    ...
    if auditor_path.resolve() != Path(__file__).resolve():
        raise ValueError("Stage58b independent auditor path differs")
    if runner.common.file_sha256(auditor_path) != str(descriptor["sha256"]).upper():
        raise ValueError("Stage58b independent auditor hash differs")
    ...
    if summary.get("status") != "stage58b_ppard_pilot96_unidock_matrix_ok":
        raise ValueError("Stage58b source matrix did not pass")
    if str(summary["config"]["sha256"]).upper() != runner.common.file_sha256(config_path):
        raise ValueError("Stage58b source config hash differs")
    if any(int(value) != 0 for value in dict(summary["data_boundary"]).values()):
        raise ValueError("Stage58b source crossed a data boundary")
    ...
    for seed in dict(config["inputs"])["seeds"]:
        ...
        for receptor in receptors:
            ...
            if batch_summary.get("status") != "ok":
                raise ValueError(f"Stage58b batch failed: {seed_id}/{receptor_id}")
            ...
            if batch_summary.get("signature") != signature:
                raise ValueError(f"Stage58b signature differs: {seed_id}/{receptor_id}")
            if runner.common.file_sha256(paths["scores"]) != str(
                batch_summary["scores_sha256"]
            ).upper():
                raise ValueError(f"Stage58b score hash differs: {seed_id}/{receptor_id}")
            if runner.common.file_sha256(paths["log"]) != str(
                batch_summary["log_sha256"]
            ).upper():
                raise ValueError(f"Stage58b log hash differs: {seed_id}/{receptor_id}")
            if len(rows) != len(ligands) or {row["ligand_id"] for row in rows} != ligand_ids:
                raise ValueError(f"Stage58b batch ligand grid differs: {seed_id}/{receptor_id}")
            ...
            if pose_audit != dict(batch_summary["pose_integrity_audit"]):
                raise ValueError(f"Stage58b pose audit differs: {seed_id}/{receptor_id}")
            warning = runner.common.classify_warning_log(paths["log"], pose_audit)
            if warning != dict(batch_summary["warning_adjudication"]):
                raise ValueError(f"Stage58b warning audit differs: {seed_id}/{receptor_id}")
            if int(warning["unresolved_warning_event_count"]) != 0:
                raise ValueError(f"Stage58b unresolved warning: {seed_id}/{receptor_id}")
            ...
            for source, audited in zip(rows, audited_rows, strict=True):
                ...
                if runner.common.file_sha256(output) != source["output_pose_sha256"].upper():
                    raise ValueError(f"Stage58b pose hash differs: {source['ligand_id']}")
                score, pose_count = prior.parse_vina_pose(output)
                if not math.isfinite(score) or abs(score - float(source["gpu_score"])) > 1e-9:
                    raise ValueError(f"Stage58b pose score differs: {source['ligand_id']}")
                if pose_count != 1 or int(source["pose_count"]) != 1:
                    raise ValueError(f"Stage58b pose count differs: {source['ligand_id']}")
                if audited["pose_integrity_status"] != "ok":
                    raise ValueError(f"Stage58b pose integrity failed: {source['ligand_id']}")
    ...
    if batch_count != int(expected["batch_count"]) or len(recomputed_rows) != int(
        ...
        raise ValueError("Stage58b independent matrix dimensions differ")
    ...
    if len(batch_rows) != batch_count or any(
        ...
        raise ValueError("Stage58b batch ledger technical gate differs")
    ...
    if int(summary["known_warning_event_count"]) != known_warnings:
        raise ValueError("Stage58b known-warning total differs")
    if int(summary["unresolved_warning_event_count"]) != 0 or int(
        summary["pose_integrity_failure_count"]
    ) != 0:
        raise ValueError("Stage58b source technical totals differ")

    result = {
        "schema_version": "1.0",
        "audit_id": "stage58b-ppard-pilot96-unidock113-independent-matrix-audit-v1",
        "status": "independent_stage58b_ppard_pilot96_unidock_matrix_audit_ok",
        ...
        "next_gate": "apply the preregistered Stage59 PPARD functional-complementarity gate",
        "decision_boundary": (
            "This audit establishes only the prospective PPARD Pilot-96 score matrix. "
            "It does not by itself establish functional complementarity, QUBO benefit, "
            "independent validation, quantum execution, speedup, or quantum advantage."
        ),
    }
