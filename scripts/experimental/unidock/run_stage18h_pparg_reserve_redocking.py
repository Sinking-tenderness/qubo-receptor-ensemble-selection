"""Run the post-hoc PPARG eight-reserve, three-seed recovery gate."""
...
def verify_implementation(root: Path, config: dict[str, object], key: str, expected: Path) -> None:
    ...
    if path.resolve() != expected.resolve():
        raise ValueError(f"Stage 18h implementation path differs: {key}")
    if common.file_sha256(path) != str(descriptor["sha256"]).upper():
        raise ValueError(f"Stage 18h implementation hash differs: {key}")
...
def validate_inputs(
    ...
    if any(int(value) != 0 for value in dict(config["data_boundary"]).values()):
        raise ValueError("Stage 18h crossed a frozen data boundary")
    ...
    adjudication = common.read_json(paths["stage18e_failure_adjudication"])
    if adjudication.get("status") != "stage18e_pparg_confirmatory_technical_gate_closed":
        raise ValueError("Stage 18e failure adjudication is not closed")
    if adjudication["outcome"]["confirmatory_gate_pass"] is not False:
        raise ValueError("Stage 18e failed status differs")
    if int(adjudication["outcome"]["passed_receptor_count"]) != 14:
        raise ValueError("Stage 18e passing-receptor count differs")
    if adjudication["decision"]["posthoc_structural_reserve_recovery_authorized"] is not True:
        raise ValueError("Stage 18h reserve recovery is not authorized")

    amendment = common.read_json(paths["reserve_selection_amendment"])
    if amendment.get("amendment_status") != "stage18f_pparg_structural_reserves_amendment01_applied":
        raise ValueError("Stage 18f technical amendment did not pass")
    if int(amendment["data_use_audit"]["reserve_redocking_jobs_started"]) != 0:
        raise ValueError("Stage 18f amendment was made after reserve docking started")
    amended_selection = common.read_csv(paths["reserve_selection_manifest"])
    amended_ids = [row["conformer_id"] for row in amended_selection]
    if amended_ids != [str(value) for value in amendment["reserve_receptor_ids"]]:
        raise ValueError("Stage 18f amended reserve manifest differs")

    preparation = common.read_json(paths["preparation_summary"])
    common_box = common.read_json(paths["common_box"])
    if preparation.get("status") != "stage18g_pparg_reserve_redocking_inputs_ok":
        raise ValueError("Stage 18g PPARG reserve input preparation did not pass")
    if any(int(value) != 0 for value in dict(preparation["data_boundary"]).values()):
        raise ValueError("Stage 18g preparation crossed a data boundary")
    for key in ("receptor_manifest_csv", "redocking_case_manifest_csv", "common_box_json"):
        source = dict(preparation["outputs"])[key]
        source_path = common.rooted_path(root, str(source["path"]))
        if common.file_sha256(source_path) != str(source["sha256"]).upper():
            raise ValueError(f"Stage 18g output identity differs: {key}")
    ...
    if receptor_ids != [str(value) for value in expected["receptor_ids"]]:
        raise ValueError("Stage 18h receptor order differs")
    if receptor_ids != amended_ids:
        raise ValueError("Stage 18h receptors differ from the Stage 18f amendment")
    if len(receptors) != int(expected["receptor_count"]) or len(cases) != int(expected["case_count"]):
        raise ValueError("Stage 18h receptor or case count differs")
    if [row["conformer_id"] for row in cases] != receptor_ids:
        raise ValueError("Stage 18h receptor and cognate-case order differs")
    if len({row["case_id"] for row in cases}) != len(cases):
        raise ValueError("Stage 18h case IDs are not unique")
    if any(row["status"] != "ok" for row in receptors + cases):
        raise ValueError("Stage 18h input manifest contains a failed row")
    if "PPARG_3WJ4_aligned" in receptor_ids or any(row["selected_ligand_resname"] == "TBY" for row in cases):
        raise ValueError("Stage 18h retained the technically incompatible TBY/Sn case")
    for row in receptors:
        ...
        if not path.is_file() or common.file_sha256(path) != row["receptor_pdbqt_sha256"].upper():
            raise ValueError(f"Stage 18h receptor identity differs: {row['conformer_id']}")
    for row in cases:
        for path_key, hash_key in (("ligand_pdbqt", "ligand_pdbqt_sha256"), ("reference_sdf", "reference_sdf_sha256")):
            ...
            if not path.is_file() or common.file_sha256(path) != row[hash_key].upper():
                raise ValueError(f"Stage 18h case identity differs: {row['case_id']}")
        if common.macrocycle_closure_atom_types(common.rooted_path(root, row["ligand_pdbqt"])):
            raise ValueError(f"Stage 18h ligand retains closure pseudoatoms: {row['case_id']}")
    seeds = tuple((str(value["seed_id"]), int(value["base_seed"])) for value in inputs["seeds"])
    if seeds != EXPECTED_SEEDS:
        raise ValueError("Stage 18h seed ledger differs")
    if int(expected["redocking_pair_count"]) != len(receptors) * len(seeds):
        raise ValueError("Stage 18h redocking pair count differs")
...
def summarize_gate(
    ...
    for receptor_id in receptor_ids:
        ...
        if len(selected) != len(EXPECTED_SEEDS):
            raise ValueError(f"Stage 18h receptor result count differs: {receptor_id}")
...
def run(config_path: Path, root: Path, unidock: str | None, audit_only: bool, resume: bool) -> dict[str, object]:
    ...
    for case in cases:
        ...
        for seed in dict(config["inputs"])["seeds"]:
            ...
            common.run_checked([
                sys.executable,
                str(common.rooted_path(root, str(dict(config["inputs"])["evaluate_rmsd_script"]["path"]))),
                "--case-id", f"{case['case_id']}__{seed_id}",
                "--reference-sdf", str(common.rooted_path(root, case["reference_sdf"])),
                "--docked-pdbqt", str(output_pose),
                "--pose-table-output", str(pose_table),
                "--summary-output", str(rmsd_summary_path),
                "--success-threshold", str(threshold),
            ], f"PPARG reserve redocking RMSD evaluation for {case['case_id']} {seed_id}")
    ...
    if len(all_rows) != expected_pairs or len({(row["conformer_id"], row["seed_id"]) for row in all_rows}) != expected_pairs:
        raise ValueError("Stage 18h complete redocking grid differs")
    ...
    minimum_passing = int(gate["minimum_new_passing_receptor_count"])
    status = (
        "stage18h_pparg_posthoc_reserve_recovery_gate_ok"
        if passed >= minimum_passing and unresolved == 0 and pose_failures == 0
        else "stage18h_pparg_posthoc_reserve_recovery_gate_failed"
    )
    ...
    summary = {
        ...
        "passed_reserve_receptor_count": passed,
        "failed_reserve_receptor_count": len(gate_rows) - passed,
        "minimum_new_passing_receptor_count": minimum_passing,
        "exploratory_recovery_gate_pass": status.endswith("_ok"),
        ...
        "next_gate": (
            "combine the 14 Stage 18e passers with the Stage 18h passers and freeze an exploratory final 16 by structural distance"
            if status.endswith("_ok")
            else "stop PPARG benchmark docking and preserve the failed exploratory recovery"
        ),
