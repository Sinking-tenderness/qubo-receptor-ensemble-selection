"""Build the bounded Stage 6 Vina-GPU search-depth diagnostic bundle."""
...
try:
    ...
    from .run_vinagpu_equivalence import read_json
    from .run_vinagpu_search_depth_diagnostic import validate_diagnostic_inputs
...
FIXED_PATHS = (
    ...
    "scripts/experimental/vinagpu/run_vinagpu_search_depth_diagnostic.py",
    "scripts/experimental/vinagpu/package_vinagpu_search_depth_results.py",
    "scripts/experimental/vinagpu/run_vinagpu_search_depth_remote.sh",
    "reports/stage-06/mk14_vinagpu21_train160_result.md",
    "reports/stage-06/mk14_vinagpu21_deterministic_batch_bridge.md",
    "reports/stage-06/mk14_vinagpu21_search_depth_diagnostic.md",
)


def bundle_paths(root: Path) -> list[str]:
    config = read_json(root / CONFIG)
    _, _, _, _, audit = validate_diagnostic_inputs(root.resolve(), config)
    if audit["validation_rows"] != 0 or audit["test_rows"] != 0:
        raise ValueError("search-depth bundle crossed a data boundary")
...
def main() -> int:
    ...
    result.update(
        {
            "operation": "targeted consumed-Train-160 Vina-GPU search-depth diagnostic input bundle",
            "receptor_count": 5,
            "ligand_count": 160,
            "diagnostic_group_count": 2,
            "pair_count_per_profile": 320,
            "chunk_count_per_profile": 40,
            "search_depth_profiles": [16, 24, 32],
            "validation_rows": 0,
            "test_rows": 0,
