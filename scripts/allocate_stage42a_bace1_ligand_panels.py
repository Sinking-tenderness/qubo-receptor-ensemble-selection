"""Freeze scaffold-disjoint BACE1 train, validation, and test ligand panels."""
...
def scaffold_for(molecule: Chem.Mol) -> str:
    # The allocation uses achiral scaffolds.  Remove stereo before and after
    # side-chain deletion so newer RDKit releases cannot retain a dangling
    # double-bond stereo annotation on the generated Murcko scaffold.
    achiral_molecule = Chem.Mol(molecule)
    Chem.RemoveStereochemistry(achiral_molecule)
    scaffold = MurckoScaffold.GetScaffoldForMol(achiral_molecule)
...
def read_decoys(path: Path) -> list[dict[str, object]]:
    ...
    with path.open("r", encoding="utf-8-sig") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if len(fields) < 2:
                raise ValueError(f"BACE1 decoy row is incomplete: {line_number}")
            molecule = Chem.MolFromSmiles(fields[0])
            if molecule is None:
                raise ValueError(f"BACE1 decoy SMILES does not parse: {line_number}")
            rows.append({
...
def group_id(group: list[dict[str, object]]) -> str:
    ...
    return f"BACE1_DECOY_G{digest.upper()}"
...
def select_exact_groups(
    ...
    for group in ranked:
        ...
        if count == target:
            break
    if count != target:
        raise ValueError(f"cannot construct exact BACE1 decoy panel of size {target}")
...
def run(config_path: Path, root: Path, overwrite: bool) -> dict[str, object]:
    ...
    if file_sha256(Path(__file__)) != str(config["implementation"]["sha256"]).upper():
        raise ValueError("Stage 42a implementation SHA-256 differs")
    inputs = {
    ...
    if len(active_scaffolds) != len(active_rows):
        raise ValueError("BACE1 active scaffold identity differs")

    decoy_rows = read_decoys(inputs["decoy_ism"])
    if len(decoy_rows) != int(config["expected"]["source_decoy_count"]):
        raise ValueError("BACE1 decoy source count differs")
    ...
    if not eligible_groups or sum(len(group) for group in eligible_groups) < sum(
        int(value) for key, value in dict(config["decoy_allocation"]).items()
        if key.endswith("_decoy_count")
    ):
        raise ValueError("BACE1 eligible decoy pool is too small")
        ...
        for group in selected_by_role[role]:
            ...
            for row in group:
                decoy_selected_rows.append({
                    **row,
                    "ligand_id": f"BACE1_decoy_L{int(row['source_line_number']):06d}",
    ...
    if observed_counts != Counter(expected_counts):
        raise ValueError("BACE1 selected panel counts differ")
    for key in ("ligand_id", "canonical_smiles", "split_group_id"):
        ...
        if any(len(splits) != 1 for splits in split_sets.values()):
            raise ValueError(f"BACE1 {key} crosses a panel boundary")
    ...
    if any(len(splits) != 1 for splits in source_splits.values()):
        raise ValueError("BACE1 source molecule ID crosses a panel boundary")
    ...
    if any(len(splits) != 1 for splits in scaffold_splits.values()):
        raise ValueError("BACE1 Bemis-Murcko scaffold crosses a panel boundary")
    ...
    if not overwrite and any(path.exists() for path in (selected_path, train_path, summary_path)):
        raise FileExistsError("Stage 42a outputs exist; pass --overwrite")
    ...
    summary = {
        ...
        "status": "stage42a_bace1_ligand_panels_frozen",
        ...
        "next_gate": "prepare only the frozen Train-266 ligands for Uni-Dock",
