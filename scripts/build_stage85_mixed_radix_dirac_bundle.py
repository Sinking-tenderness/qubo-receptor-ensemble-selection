"""Build deterministic Stage85 external-input or returned-result archives."""
...
def input_paths(root: Path) -> list[Path]:
    fixed = [
        ...
        "configs/stage85_mixed_radix_dirac_calibration.json",
        "data/stage72_constraint_native_cqm_model_record.json",
        "data/stage85_mixed_radix_dirac_calibration_prepared.json",
        "data/stage85_mixed_radix_dirac_calibration_audit.json",
        "environment/stage79_qci_dirac3.yml",
        "reports/stage-85/mixed_radix_dirac_external_execution.md",
        ...
        "scripts/build_stage85_mixed_radix_dirac_bundle.py",
    ]
    paths = [root / value for value in fixed]
    paths.extend(
        sorted(
            (root / "results/runs/stage85_mixed_radix_dirac_calibration/instances").glob(
                "*.json"
            )
        )
    )
...
def result_paths(root: Path) -> list[Path]:
    paths = [
        root / "configs/stage85_mixed_radix_dirac_calibration.json",
        root / "data/stage85_mixed_radix_dirac_calibration_prepared.json",
    ]
    output = root / "external_results/stage85_mixed_radix_dirac_calibration"
    if output.is_dir():
        paths.extend(sorted(path for path in output.rglob("*") if path.is_file()))
    for name in ("stage85_qci_preflight.log", "stage85_qci_calibration.log"):
        path = root / name
        if path.is_file():
            paths.append(path)
    return paths
...
def build(root: Path, output: Path, kind: str) -> dict[str, object]:
    ...
    if missing:
        raise FileNotFoundError(f"Stage85 bundle is missing files: {missing}")
    ...
    manifest = ("\n".join(manifest_lines) + "\n").encode("ascii")
    prefix = (
        "stage85_mixed_radix_dirac_calibration_external_input_v1"
        if kind == "input"
        else "stage85_mixed_radix_dirac_calibration_results_v1"
    )
