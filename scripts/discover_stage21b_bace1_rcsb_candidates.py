"""Discover label-independent RCSB receptor candidates for BACE1."""
...
def write_report(path: Path, summary: dict[str, object]) -> None:
    ...
    lines = [
        "# Stage 21b BACE1 RCSB Candidate Discovery",
        "",
        "- RCSB P56817 X-ray entries: " + str(summary["query"]["search_result_count"]),
        "- Metadata-eligible entries: " + str(counts["metadata_eligible_count"]),
        "- Eligible new entries: " + str(counts["new_metadata_eligible_count"]),
        "- 3L5D reference eligible: " + str(counts["reference_eligible"]).lower(),
        "",
        "TAR and nonpolymers below 0.20 kDa were excluded so crystallization additives cannot qualify a structure.",
...
def run(config_path: Path, root: Path, overwrite: bool) -> dict[str, object]:
    ...
    if file_sha256(Path(__file__)) != str(config["implementation"]["sha256"]).upper():
        raise ValueError("Stage 21b implementation SHA-256 differs")
    ...
    if (
        ...
        != "stage21a_bace1_source_and_active_allocation_ok"
    ):
        raise ValueError("Stage 21a BACE1 source audit did not pass")
    ...
    if any(path.exists() for path in outputs.values()) and not overwrite:
        raise FileExistsError("Stage 21b outputs exist; pass --overwrite")
    ...
    accession = str(preregistration["target"]["uniprot_accession"])
    ...
    if not identifiers:
        raise ValueError("BACE1 RCSB search returned no structures")
    ...
    for start in range(0, len(identifiers), chunk_size):
        ...
        if response.get("errors"):
            raise ValueError(f"Stage 21b RCSB GraphQL errors: {response['errors']}")
        data = response.get("data")
        if not isinstance(data, dict):
            raise ValueError("Stage 21b RCSB GraphQL data are missing")
    ...
    if {str(row.get("rcsb_id", "")).upper() for row in entries} != set(identifiers):
        raise ValueError("BACE1 RCSB metadata IDs differ from the search response")

    eligibility = dict(config["discovery_rules"]["metadata_eligibility"])
    reference_id = str(preregistration["target"]["dude_reference_structure"]).upper()
    rows = [
        apply_title_exclusions(
    ...
    if len(reference_rows) != 1 or reference_rows[0]["status"] != "metadata_eligible":
        raise ValueError("BACE1 DUD-E reference is not metadata eligible")
    if len(eligible) < int(config["discovery_rules"]["minimum_metadata_eligible_count"]):
        raise ValueError("too few BACE1 metadata-eligible structures")
    ...
    summary = {
        ...
        "status": "stage21b_bace1_metadata_discovery_ok",
        "config": {
        ...
        "next_gate": "coordinate download, chain audit, proper-rotation alignment, pocket completeness, pocket-proximal ligand check, preparation-readiness audit, and deterministic max-min selection",
