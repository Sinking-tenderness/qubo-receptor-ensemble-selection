"""Prepare and geometrically audit post-hoc PPARG reserve redocking inputs."""
...
try:
    ...
    from scripts.prepare_stage14c_fa10_redocking_inputs import (
    ...
    from prepare_stage14c_fa10_redocking_inputs import (
        alignment_audit_fa10,
        coordinates_and_elements_from_sdf_safe,
        prepare_ligand_safe,
        transform_sdf_safe,
...
def run(config_path: Path, root: Path, overwrite: bool) -> dict[str, object]:
    ...
    if file_sha256(Path(__file__)) != str(config["implementation"]["sha256"]).upper():
        raise ValueError("Stage 18g implementation SHA-256 differs")
    ...
    if selection["status"] != "stage18f_pparg_structural_reserves_selected":
        raise ValueError("PPARG structural reserve selection did not pass")
    audit = dict(selection["data_use_audit"])
    if any(
        int(audit[key]) != 0
        for key in (
            "activity_labels_read",
            "benchmark_docking_scores_read",
            "fresh_validation_rows_read",
            "test_rows_read",
        )
    ):
        raise ValueError("PPARG structural reserve selection crossed a protected boundary")
    selected_rows = read_csv(inputs["selected_receptor_manifest"])
    if len(selected_rows) != int(config["expected"]["receptor_count"]):
        raise ValueError("PPARG selected receptor count differs")
    if [row["conformer_id"] for row in selected_rows] != selection["reserve_receptor_ids"]:
        raise ValueError("PPARG selected receptor order differs")
    if any(int(row["global_incomplete_standard_amino_acid_residue_count"]) for row in selected_rows):
        raise ValueError("PPARG selected pool contains an incomplete residue")
    if any(row["explicit_covalent_connections"].strip() for row in selected_rows):
        raise ValueError("PPARG selected pool contains an explicit covalent connection")
    ...
    if any(outputs[key].exists() for key in file_keys) and not overwrite:
        raise FileExistsError("Stage 18g outputs exist; pass --overwrite")

    reference = dict(config["reference"])
    anchors = [int(value) for value in reference["anchor_residue_numbers"]]
    ...
    for selected in selected_rows:
        ...
        alignment["target"] = "PPARG"
        alignment["method"] = "proper-rotation Kabsch alignment over all shared, residue-name-matched PPARG C-alpha atoms"
        ...
        command = [
            sys.executable, str(inputs["prepare_receptor_script"]),
            "--input-pdb", str(aligned_pdb), "--chain", "A",
            "--protein-only-output", str(protein_only),
            "--prepared-pdb-output", str(prepared_pdb),
            "--pdbqt-output", str(receptor_pdbqt),
            "--summary-output", str(summary_path),
        ]
        if overwrite:
            command.append("--overwrite")
        run_checked(command, f"PPARG receptor preparation for {conformer_id}")
        ...
        if receptor_evidence.get("status") != "ok" or receptor_evidence.get("allow_bad_res") is not False:
            raise ValueError(f"PPARG receptor preparation failed: {conformer_id}")
        ...
        if residue_change["input_protein_only"] != residue_change["output_pdbqt"]:
            raise ValueError(f"PPARG receptor residue count changed: {conformer_id}")

        source_url = build_modelserver_url(
            str(download["url_template"]), pdb_id, selected["chain"],
            int(selected["selected_ligand_resseq"]),
        )
        raw_sdf = case_root / f"{case_id}_raw.sdf"
        download_cached(
            source_url, raw_sdf, float(download["timeout_seconds"]),
            int(download["maximum_retries"]), float(download["retry_backoff_seconds"]),
        )
        raw_coordinates, raw_elements, raw_molecule = coordinates_and_elements_from_sdf_safe(raw_sdf)
        ligand_key = (
            ligand_resname, int(selected["selected_ligand_resseq"]),
            selected["selected_ligand_icode"],
        )
        raw_ligands = ligand_residue_map(raw_atoms, {ligand_resname})
        if ligand_key not in raw_ligands:
            raise ValueError(f"selected PPARG mmCIF ligand is missing: {case_id}")
        ...
        if raw_molecule.GetNumHeavyAtoms() != int(selected["selected_ligand_heavy_atom_count"]):
            raise ValueError(f"PPARG ligand heavy-atom count differs: {case_id}")
        ...
        if raw_rmsd > limit or raw_maximum > limit:
            raise ValueError(f"PPARG ModelServer ligand coordinates differ: {case_id}")
        ...
        transform_sdf_safe(raw_sdf, common_sdf, common_summary, rotation, translation, False)
        transform_sdf_safe(raw_sdf, explicit_h_sdf, explicit_h_summary, rotation, translation, True)
        ...
        if common_rmsd > limit or common_maximum > limit:
            raise ValueError(f"PPARG common-frame ligand coordinates differ: {case_id}")
        ...
        receptor_rows.append({
            "conformer_id": conformer_id, "pdb_id": pdb_id, "chain": "A",
            ...
            "receptor_preparation_summary_sha256": file_sha256(summary_path), "status": "ok",
        })
        case_rows.append({
            "case_id": case_id, "conformer_id": conformer_id, "pdb_id": pdb_id,
            "source_chain": selected["chain"], "selected_ligand_resname": ligand_key[0],
            "selected_ligand_resseq": ligand_key[1], "selected_ligand_icode": ligand_key[2],
            "selected_ligand_heavy_atom_count": common_molecule.GetNumHeavyAtoms(),
            "source_url": source_url, "raw_sdf": raw_sdf.relative_to(root).as_posix(),
            ...
            "alignment_summary_sha256": file_sha256(alignment_path), "status": "ok",
        })

    fixed_box = read_json(inputs["fixed_common_box"])
    coordinates = np.vstack(common_coordinate_sets)
    center = np.asarray(
        [float(fixed_box["center"][axis]) for axis in ("x", "y", "z")],
        dtype=float,
    )
    size = np.asarray(
        [float(fixed_box["size"][axis]) for axis in ("x", "y", "z")],
        dtype=float,
    )
    lower = center - size / 2.0
    upper = center + size / 2.0
    minimum_margin = float(
        min(np.min(coordinates - lower), np.min(upper - coordinates))
    )
    if minimum_margin < float(config["fixed_box_audit"]["minimum_allowed_margin_angstrom"]):
        raise ValueError("a PPARG reserve crystal ligand falls outside the frozen box")
    box = {
        "center": dict(fixed_box["center"]),
        "size": dict(fixed_box["size"]),
        "ligand_minimum_angstrom": coordinates.min(axis=0).tolist(),
        "ligand_maximum_angstrom": coordinates.max(axis=0).tolist(),
        "minimum_observed_margin_angstrom": minimum_margin,
    }
    box_record = {
        "schema_version": "1.0", "status": "stage18g_pparg_reserve_common_box_ok",
        "source_fixed_box": {
            "path": inputs["fixed_common_box"].relative_to(root).as_posix(),
            "sha256": file_sha256(inputs["fixed_common_box"]),
        },
        "receptor_count": len(selected_rows), **box,
    }
    ...
    result = {
        ...
        "status": "stage18g_pparg_reserve_redocking_inputs_ok",
        "config": {"path": config_path.relative_to(root).as_posix(), "sha256": file_sha256(config_path)},
        "counts": {"selected_receptor_count": len(selected_rows), "prepared_receptor_count": len(receptor_rows), "prepared_cognate_ligand_count": len(case_rows), "failed_case_count": 0},
        "common_box": box_record,
        "maximum_raw_ligand_coordinate_rmsd_angstrom": max(float(row["raw_sdf_to_mmcif_element_matched_rmsd_angstrom"]) for row in case_rows),
        "maximum_common_ligand_coordinate_rmsd_angstrom": max(float(row["common_sdf_to_transformed_mmcif_element_matched_rmsd_angstrom"]) for row in case_rows),
        "data_boundary": {"ligand_labels_read": 0, "benchmark_docking_scores_read": 0, "fresh_validation_rows_read": 0, "test_rows_read": 0},
        "outputs": {
            key: {"path": outputs[key].relative_to(root).as_posix(), "sha256": file_sha256(outputs[key])}
            for key in ("receptor_manifest_csv", "redocking_case_manifest_csv", "common_box_json")
        },
        "next_gate": "run the eight-reserve by three-seed PPARG Uni-Dock recovery gate",
