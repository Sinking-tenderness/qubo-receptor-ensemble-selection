"""Create a non-destructive PPARD target-id amendment for Stage58b scores."""
...
def verified(root: Path, value: dict[str, Any]) -> Path:
    ...
    if not path.is_file() or sha256(path) != str(value["sha256"]).upper():
        raise ValueError(f"Stage58c input identity differs: {path}")
    return path
...
def run(config_path: Path, root: Path, overwrite: bool) -> dict[str, Any]:
    ...
    if implementation.resolve() != Path(__file__).resolve():
        raise ValueError("Stage58c implementation path differs")
    ...
    if matrix_audit.get("status") != (
        ...
        raise ValueError("Stage58b independent audit did not pass")
    if summary.get("status") != "stage58b_ppard_pilot96_unidock_matrix_ok":
        raise ValueError("Stage58b production summary did not pass")
    if int(matrix_audit["pair_count"]) != 8352 or int(
        matrix_audit["pose_integrity_failure_count"]
    ) != 0:
        raise ValueError("Stage58b technical matrix dimensions differ")
    ...
    if len(rows) != int(expected["row_count"]):
        raise ValueError("Stage58c row count differs")
    if {row["target_id"] for row in rows} != {expected["observed_target_id"]}:
        raise ValueError("Stage58c observed target-id defect differs")
    ...
    if [row_fingerprint(row) for row in amended] != before:
        raise ValueError("Stage58c changed non-target metadata")
    changed_fields = sum(
        sum(source[key] != target[key] for key in source)
        for source, target in zip(rows, amended, strict=True)
    )
    if changed_fields != len(rows):
        raise ValueError("Stage58c did not change exactly one field per row")
    ...
    if not overwrite and (output_path.exists() or result_path.exists()):
        raise FileExistsError("Stage58c outputs exist; pass --overwrite")
    ...
    if [row_fingerprint(row) for row in reread] != before:
        raise ValueError("Stage58c output fingerprints differ")
    if {row["target_id"] for row in reread} != {expected["corrected_target_id"]}:
        raise ValueError("Stage58c output target ID differs")

    result = {
        "schema_version": "1.0",
        "status": "stage58c_ppard_target_id_amendment_ok",
