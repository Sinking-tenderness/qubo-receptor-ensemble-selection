"""Discover label-independent RCSB receptor candidates for PPARG."""
...
def write_report(path: Path, summary: dict[str, object]) -> None:
    ...
    lines = [
        "# Stage 18b PPARG RCSB Candidate Discovery",
        "",
        "- RCSB P37231 X-ray entries: " + str(summary["query"]["search_result_count"]),
        "- Metadata-eligible entries: " + str(counts["metadata_eligible_count"]),
        "- Eligible new entries: " + str(counts["new_metadata_eligible_count"]),
        "- 2GTK reference eligible: " + str(counts["reference_eligible"]).lower(),
        "",
        "No PPARG docking score, fresh-validation row, or test row was read.",
...
def run(config_path: Path, root: Path, overwrite: bool) -> dict[str, object]:
    ...
    if file_sha256(Path(__file__)) != str(config["implementation"]["sha256"]).upper():
        raise ValueError("Stage 18b implementation SHA-256 differs")
    ...
    if read_json(source_audit_path)["status"] != "stage18a_pparg_source_and_active_allocation_ok":
        raise ValueError("Stage 18a PPARG source audit did not pass")
    if any(
    ...
    if any(path.exists() for path in outputs.values()) and not overwrite:
        raise FileExistsError("Stage 18b outputs exist; pass --overwrite")
    ...
    if not identifiers:
        raise ValueError("PPARG RCSB search returned no structures")
    ...
    for start in range(0, len(identifiers), chunk_size):
        ...
        if response.get("errors"):
            raise ValueError(f"Stage 18b RCSB GraphQL errors: {response['errors']}")
        data = response.get("data")
        if not isinstance(data, dict):
            raise ValueError("Stage 18b RCSB GraphQL data are missing")
        entries.extend(
            row for row in data.get("entries") or [] if isinstance(row, dict)
        )
    entries.sort(key=lambda row: str(row.get("rcsb_id", "")))
    if {str(row.get("rcsb_id", "")).upper() for row in entries} != set(identifiers):
        raise ValueError("PPARG RCSB metadata IDs differ from the search response")
    ...
    if len(reference_rows) != 1 or reference_rows[0]["status"] != "metadata_eligible":
        raise ValueError("PPARG DUD-E reference is not metadata eligible")
    if len(eligible) < int(config["discovery_rules"]["minimum_metadata_eligible_count"]):
        raise ValueError("too few PPARG metadata-eligible structures")
    ...
    summary = {
        ...
        "status": "stage18b_pparg_metadata_discovery_ok",
        "config": {
