"""Prepare and geometrically audit the PPARG cognate-redocking inputs."""
...
def run(config_path: Path, root: Path, overwrite: bool) -> dict[str, object]:
    ...
    if file_sha256(Path(__file__)) != str(config["implementation"]["sha256"]).upper():
        raise ValueError("Stage 18d implementation SHA-256 differs")
    ...
    if selection["status"] != "stage18c_pparg_preparation_ready_pool_ok":
        raise ValueError("PPARG preparation-ready structural selection did not pass")
    if any(int(value) != 0 for value in selection["data_boundary"].values()):
        raise ValueError("PPARG structural selection crossed a protected boundary")
    ...
    if any(outputs[key].exists() for key in file_keys) and not overwrite:
        raise FileExistsError("Stage 18d outputs exist; pass --overwrite")
    ...
    box = derive_common_box(
        common_coordinate_sets, float(box_rule["minimum_crystal_pose_margin_angstrom"]),
        float(box_rule["size_increment_angstrom"]), float(box_rule["minimum_axis_size_angstrom"]),
        int(box_rule["center_decimal_places"]),
    )
    if max(box["size"].values()) > float(box_rule["maximum_axis_size_angstrom"]):
        raise ValueError("PPARG common box is too large")
    box_record = {
        "schema_version": "1.0", "status": "stage18d_pparg_common_box_ok",
        "derivation_rule": box_rule, "receptor_count": len(selected_rows), **box,
    }
    ...
    result = {
        ...
        "status": "stage18d_pparg_redocking_inputs_ok",
        ...
        "next_gate": "run the 24-receptor by three-seed PPARG Uni-Dock cognate-redocking bundle",
