"""Download, audit, and select a preparation-ready BACE1 structural pool."""
...
def conformer_id(pdb_id: str) -> str:
    return "BACE1_3L5D_reference" if pdb_id == "3L5D" else f"BACE1_{pdb_id}_aligned"


def write_report(path: Path, summary: dict[str, object]) -> None:
    counts = dict(summary["counts"])
    lines = [
        "# Stage 21c BACE1 Structural Selection",
        "",
        ...
        "No BACE1 docking score, held-out row, or PPARG Stage 19c enrichment row was read.",
        "",
    ]
...
def run(config_path: Path, root: Path, overwrite: bool) -> dict[str, object]:
    ...
    if file_sha256(Path(__file__)) != str(config["implementation"]["sha256"]).upper():
        raise ValueError("Stage 21c implementation SHA-256 differs")
    ...
    if discovery["status"] != "stage21b_bace1_metadata_discovery_ok":
        raise ValueError("Stage 21b BACE1 metadata discovery did not pass")
    if any(int(value) != 0 for value in dict(discovery["data_boundary"]).values()):
        raise ValueError("Stage 21b crossed a protected data boundary")
    boundary = preregistration["data_boundary"]
    if any(
        int(boundary[key]) != 0
        for key in (
            "BACE1_benchmark_docking_scores_read",
            "BACE1_fresh_validation_rows_read",
            "BACE1_test_rows_read",
            "PPARG_stage19c_enrichment_rows_read",
        )
    ):
        raise ValueError("Stage 21c crossed a protected data boundary")
    ...
    if len(candidates) != int(discovery["counts"]["metadata_eligible_count"]):
        raise ValueError("BACE1 metadata-candidate count differs")
    if len({row["pdb_id"] for row in candidates}) != len(candidates):
        raise ValueError("BACE1 metadata candidates contain duplicate PDB IDs")
    ...
    if any(path.exists() for path in protected_outputs) and not overwrite:
        raise FileExistsError("Stage 21c outputs exist; pass --overwrite")
    ...
    for metadata in sorted(candidates, key=lambda row: row["pdb_id"]):
        ...
        if pdb_id in errors:
            ...
            continue
        aligned_path = aligned_directory / f"{candidate_id}_to_3L5D_A.pdb"
        try:
        ...
        if row["status"] == "coordinate_eligible":
            if vector is None or names is None:
                raise ValueError("BACE1 eligible candidate has no feature vector")
            vectors[candidate_id] = vector
            if feature_names is None:
                feature_names = names
            elif feature_names != names:
                raise ValueError("BACE1 structural feature names differ")
    ...
    ready_ids = {str(row["conformer_id"]) for row in ready_rows}
    ready_vectors = {key: value for key, value in vectors.items() if key in ready_ids}
    reference_id = "BACE1_3L5D_reference"
    if reference_id not in ready_vectors:
        raise ValueError("BACE1 reference failed preparation readiness")
    target_count = int(config["structural_selection"]["target_receptor_count"])
    status = "stage21c_bace1_preparation_ready_pool_insufficient_stop"
    selected_rows: list[dict[str, object]] = []
    feature_rows: list[dict[str, object]] = []
    distance_rows: list[dict[str, object]] = []
    variable_feature_count = 0
    if len(ready_vectors) >= target_count:
        ordered_ids = sorted(ready_vectors)
        matrix = np.vstack([ready_vectors[value] for value in ordered_ids])
        ...
        if variable_feature_count < 3:
            raise ValueError("too few variable BACE1 structural features")
        ...
        ready_by_id = {str(row["conformer_id"]): row for row in ready_rows}
        selected_rows.append(
            {
                "pool_role": "reference_seed",
                "selection_rank": 1,
                "minimum_standardized_distance_to_selected_pool": "",
                **ready_by_id[reference_id],
            }
        )
            ...
            selected_rows.append(
                {
                    ...
                    **ready_by_id[str(addition["conformer_id"])],
                }
            )
        feature_rows = [
            {
                ...
                **{
                    name: float(number)
                    for name, number in zip(feature_names or [], ready_vectors[value])
                },
            }
            for value in ordered_ids
        ]
        status = "stage21c_bace1_preparation_ready_structural_pool_ok"

    write_csv(outputs["coordinate_audit_csv"], audit_rows)
    write_csv(outputs["eligible_pool_manifest_csv"], eligible_rows)
    write_csv(outputs["preparation_ready_pool_manifest_csv"], ready_rows)
    if feature_rows:
    ...
    summary = {
        ...
        "counts": {
            ...
            "preparation_ready_count": len(ready_rows),
            "globally_incomplete_coordinate_eligible_count": len(eligible_rows)
            - len(ready_rows),
        ...
        "outputs": {
            ...
            for key, path in outputs.items()
            if path.is_file() and key not in {"summary_json", "report_md"}
        },
        "next_gate": (
            "prepare the frozen 24 BACE1 receptors and cognate ligands for three-seed Uni-Dock redocking"
            if status.endswith("_ok")
            else "do not dock; close the BACE1 coordinate-pool gate"
        ),
