"""Discover label-independent RCSB receptor candidates for SRC."""
...
def verified(root: Path, record: dict[str, Any]) -> Path:
    ...
    if not path.is_file() or file_sha256(path) != record["sha256"].upper():
        raise ValueError(f"Stage46a input identity differs: {path}")
    return path
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# Stage46a SRC RCSB candidate discovery",
        "",
        f"- RCSB P12931 X-ray entries: {summary['query']['search_result_count']}",
        f"- Metadata-eligible entries: {counts['metadata_eligible_count']}",
        f"- New eligible entries: {counts['new_metadata_eligible_count']}",
        f"- 3EL8 reference eligible: {str(counts['reference_eligible']).lower()}",
        "",
        "No docking score, fresh-validation row, or locked-test row was read.",
        "Coordinate auditing and cognate redocking remain mandatory.",
        "",
    ]
...
def run(config_path: Path, root: Path, overwrite: bool) -> dict[str, Any]:
    ...
    if implementation.resolve() != Path(__file__).resolve():
        raise ValueError("Stage46a implementation path differs")
    source_path = verified(root, config["source_audit"])
    source = read_json(source_path)
    if source.get("status") != "stage46_src_source_audit_ok":
        raise ValueError("Stage46 SRC source audit did not pass")
    if not source["decision"]["structural_metadata_discovery_authorized"]:
        raise ValueError("Stage46 did not authorize metadata discovery")
    ...
    if not overwrite and any(path.exists() for path in outputs.values()):
        raise FileExistsError("Stage46a outputs exist; pass --overwrite")
    rcsb = config["rcsb"]
    request_kwargs = {
    ...
    accession = source["target"]["uniprot_accession"]
    ...
    if not identifiers:
        raise ValueError("SRC RCSB search returned no structures")
    ...
    for start in range(0, len(identifiers), chunk_size):
        ...
        if response.get("errors"):
            raise ValueError(f"Stage46a RCSB GraphQL errors: {response['errors']}")
        data = response.get("data")
        if not isinstance(data, dict):
            raise ValueError("Stage46a RCSB GraphQL data are missing")
    ...
    if {row.get("rcsb_id", "").upper() for row in entries} != set(identifiers):
        raise ValueError("SRC RCSB metadata IDs differ from search response")

    eligibility = config["discovery_rules"]["metadata_eligibility"]
    reference_id = source["target"]["dude_reference_structure"].upper()
    rows = [
        apply_title_exclusions(
    ...
    eligible_new = [row for row in eligible if row["pdb_id"].upper() != reference_id]
    reference_rows = [row for row in rows if row["pdb_id"].upper() == reference_id]
    reference_eligible = len(reference_rows) == 1 and reference_rows[0]["status"] == "metadata_eligible"
    if not reference_eligible:
        raise ValueError("SRC DUD-E reference is not metadata eligible")
    if len(eligible) < int(config["discovery_rules"]["minimum_metadata_eligible_count"]):
        raise ValueError(
            f"too few SRC metadata-eligible structures: {len(eligible)} < "
            f"{config['discovery_rules']['minimum_metadata_eligible_count']}"
        )
    ...
    summary = {
        ...
        "status": "stage46a_src_metadata_discovery_ok",
        ...
        "next_gate": "download frozen eligible coordinates and apply label-independent alignment, pocket-completeness, covalency, and preparation-readiness gates",
